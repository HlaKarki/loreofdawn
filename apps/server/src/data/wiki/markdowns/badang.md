...
After every 4 basic attacks, Badang's next Basic Attack deals scaling physical damage to enemies hit and knocks them back. Enemies that are knocked into obstacles will be stunned for 0.6s (minions and creeps are stunned directly). Damage generated by Badang's skills can also trigger this effect.
...
- It however cannot Crit and be triggered twice by the same attack.
- Heroes knocked back to a wall are actually stunned for 2 seconds. This is also the case for Minions and Creeps hit.
- The dashing speed of the 5th attack scales to Badang's Attack Speed.
- Heroes knocked back are moved a distance equal to the position of Badang and the edge of his attack range.

## Fist Wind

Badang unleashes a gust of energy in the target direction, dealing scaling physical damage to enemies hit, slowing them by 30% for 1.5s, and knocking them back. The projectile will explode upon hitting an obstacle, dealing scaling explosion damage to nearby enemies. This skill can trigger attack effects and grants Badang 1 stack upon cast. Badang stores up to 2 charges and restores 1 charge every 10.4s.
...
- This skill can't be blocked by shields or benefit from Spell Vamp, but it will trigger other Basic attack effects.
- The explosion damage does however trigger Spell Vamp.
- This skill triggers on-hit effects such as Ambush and similar Divine Justice on-cast effects.
- The included effect is regarded as skill damage and can trigger effects like Deter and benefit from Spell Vamp.
- Heroes knocked back to a wall are actually stunned for 2 seconds. This is also the case for Minions and Creeps hit.
- It can damage turrets if cast inside their range but the damage dealt is based on the projectile's stats, not the ultimate's stats.
- The projectile only explodes once even if passing through multiple obstacles.
- Units hit by the projectile right before it dissipates only take damage and slow, but are not knocked back nor inflicted with explosion damage.

## Fist Break

Badang dashes in the target direction, dealing scaling physical damage to enemies in his path while gaining a shield that lasts 5 seconds. He stops immediately upon hitting an enemy hero, slightly knocks them back while creating an obstacle that lasts 4 seconds behind them. Enemies hit by the obstacle are dealt additional scaling explosion damage. Use Again: Badang removes the obstacle he created.

- Base Shield: 350 / 400 / 450 / 500 / 550 / 600
- Base Damage: 230 / 250 / 270 / 290 / 310 / 330
- Explosion Damage: 130 / 140 / 150 / 160 / 170 / 180

Terms: Physical Damage Skill, Displacement, Cooldown

Notes:

- Badang will only create an obstacle if he encounters Heroes on his way.
- He will pass through certain summoned units or illusions when the skill is active.
...
- Unique passive Ambush triggers on the first hit of this skill.
- Certain passive effects will trigger on the first hit of this skill only if Badang has already cast another skill prior.
- This skill can only be cancelled or interrupted by Suppression during throwing.
- The included effect is regarded as skill damage and can trigger effects like Deter and benefit from Spell Vamp.
- Heroes knocked back to a wall are actually stunned for 2 seconds. This is also the case for Minions and Creeps hit.
- It can damage turrets once if cast inside their range but the damage dealt is based on the skill's stats.
- Heroes, Minions, and Creeps are only knocked back or stunned at the start of the skill.
- Projectiles only explode once even if passing through multiple obstacles.
- Units hit by projectiles at the very edge only take damage, but are not knocked back or stunned.

# Trivia

- He is based on Badang, a legendary strongman in Malay legend (see Malaysian or English sources for different language versions).
- Badang is the second hero in the game that can manipulate the map's layout, being able to create walls with his skill.
