...
Born in the Vonetis Islands off the southern part of the mainland. Originally just an ordinary villager, after defeating the water monster that was harassing the village, he gained huge power from the water monster.

After that, Badang set out to the mountains of the southern Moniyan Empire where the Abyss was entrenched, hoping to completely solve the problem of the water source being contaminated by the Abyss.
...
After every 4 Basic Attacks, Badang's next Basic Attack deals scaling physical damage to enemies hit and knocks them back. Enemies that are knocked into obstacles will be stunned for 0.6 seconds (minions and creeps are stunned directly). Displacement effects generated by Badang's skills can also trigger this effect.

Notes:
- This effect can trigger on-hit effects and benefit from Lifesteal as it is separate from the original basic attack.
- It cannot Crit or be triggered twice by the same action.
- Heroes knocked back into a wall are stunned for 2 seconds. This is also the case for Minions and Creeps hit.
- The dashing speed of the fifth hit scales with Badang's Attack Speed.
- Heroes knocked back are moved a distance equal to the position of Badang and the edge of his skill range.

## Fist Wind

Badang unleashes a gust in the target direction, dealing scaling physical damage to enemies hit, slowing them by 30% for 1.5 seconds, and knocking them back. The projectile will explode upon hitting an obstacle, dealing scaling explosion damage to nearby enemies. Fist Wind can trigger attack effects and grants Badang 1 stack upon cast. Badang stores up to 2 charges and restores 1 charge every 10.4 seconds.
...
Notes:
- The projectile cannot be affected by Lifesteal or Spell Vamp as a basic attack, but the explosion damage can trigger Spell Vamp.
- The explosion only occurs once even if passing through multiple obstacles.
- Heroes, Minions, and Creeps hit by the projectile right before it dissipates only take damage and slow, but are not knocked back nor inflicted with explosion damage.

## Fist Break

Badang dashes in the target direction, dealing scaling physical damage to enemies in his path while gaining a shield that lasts 5 seconds. He stops immediately upon hitting an enemy hero, slightly knocks them back while creating an obstacle that lasts 4 seconds behind them. Enemies hit by the obstacle are dealt scaling explosion damage. Use Again: Badang removes the obstacle he created.
...
- He will pass through certain abilities' original bodies when this skill is active.
...
Badang throws a flurry of punches in the target direction, dealing scaling physical damage multiple times over 2.5 seconds (can trigger attack effects). The projectiles generated by the punches will explode upon hitting an obstacle, dealing scaling explosion damage to nearby enemies. Badang is immune to control effects for the duration.

Properties:
- Base Damage per hit by level: 60 / 90 / 120
- Explosion Damage by level: 30 / 45 / 60
- Attack Effects: 100%

Notes:
- Certain passive effects trigger on the first hit of this skill.
- This skill can only be cancelled or interrupted by Suppression during the throwing animation.
- The skill's damage is regarded as skill damage and can trigger effects that interact with skill damage.

# Trivia

- He is based on Badang, a legendary strongman in Malay legend.
- Badang is the second hero in the game that can manipulate the map's layout, being able to create walls with his abilities.
- His Susanoo skin is based on Susanoo-no-Mikoto, a kami associated with sea, storms, field, harvest, marriage and love in Japanese mythology. The skin references Susanoo and includes the name *Susanowo* in its styling.
