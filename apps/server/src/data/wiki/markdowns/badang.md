...
After every 4 basic attacks, Badang's next Basic Attack deals scaling damage to enemies hit and knocks them back. Enemies that are knocked into obstacles will be stunned for 0.6s (minions and creeps are stunned directly). Effects generated by Badang's skills can also trigger this effect.

- Term: Physical Basic Attack, Displacement, Stun

Notes:
- Can trigger on-hit effects and benefit from Lifesteal as it is separate from the original basic attack.
- It cannot critical strike and cannot be triggered twice by the same attack.
...
Badang unleashes a gust in the target direction, dealing scaling damage to enemies hit, slowing them by 30% for 1.5s, and knocking them back. The projectile will explode upon hitting an obstacle, dealing additional scaling damage to nearby enemies. Fist Wind can trigger attack effects and grants Badang 1 stack upon cast. Badang stores up to 2 charges and restores 1 charge every 10.4s.

- Properties:
  - Base Damage: 125 / 145 / 165 / 185 / 205 / 225 (scaling damage based on skill level and attack)
  - Explosion Damage: 100 / 115 / 130 / 145 / 160 / 175 (scaling damage based on skill level and attack)
  - Skill Recharge Time: 10.4 / 9.8 / 9.2 / 8.6 / 8.0 / 7.2 seconds
  - Attack Effects: 100%

- Term: Physical Basic Attack, Attack Effect

Notes:
- Cannot critical strike or benefit from Spell Vamp, but it will trigger other basic attack effects.
- The explosion damage does trigger Spell Vamp.
- It triggers on-hit effects such as certain item or hero effects on-cast.
- The included slow effect is regarded as skill damage and can trigger effects like deter and benefit from Spell Vamp.
- Heroes knocked back to a wall are stunned for 2 seconds. This is also the case for Minions and Creeps hit.
- It can damage turrets if cast inside their range but the damage dealt is based on the projectile's base damage rather than Badang's stats.
- The projectile only explodes once even if passing through multiple obstacles.
- Units hit by the projectile right before it dissipates take damage and slow, but are not knocked back nor inflicted with explosion damage.

## Fist Break

Badang dashes in the target direction, dealing scaling damage to enemies in his path while gaining a shield that lasts 5 seconds. He stops immediately upon hitting an enemy hero, slightly knocks them back while creating an obstacle that lasts 4 seconds behind them. Enemies hit by the obstacle are dealt explosion scaling damage. Use Again: Badang removes the obstacle he created.

- Properties:
  - Base Shield: 350 / 400 / 450 / 500 / 550 / 600 (scaling shield based on skill level and stats)
  - Base Damage: 230 / 250 / 270 / 290 / 310 / 330 (scaling damage based on skill level and attack)
  - Explosion Damage: 130 / 140 / 150 / 160 / 170 / 180 (scaling damage based on skill level and attack)
...
- He will pass through some clones or illusions when this skill is active.
...
Badang throws a flurry of punches in the target direction, dealing scaling damage to enemies hit 9 times over 2.5 seconds (can trigger attack effects). The projectiles generated by the punches will explode upon hitting an obstacle, dealing additional scaling damage to nearby enemies. Badang is immune to control effects for the duration.

- Properties:
  - Base Damage per hit: 60 / 90 / 120 (scaling damage based on skill level and attack)
  - Explosion Damage: 30 / 45 / 60 (scaling damage based on skill level and attack)
  - Attack Effects: 100%

- Term: Physical Basic Attack, Control Immunity

Notes:
- Certain passive effects trigger on the first hit of this skill.
- Some passive effects will only trigger on the first hit of this skill if Badang has already cast another skill prior.
- This skill can only be cancelled or interrupted by suppression during the throwing.
- The included effects are regarded as skill damage and can trigger effects like deter and benefit from Spell Vamp.
- Heroes knocked back to a wall are stunned for 2 seconds. This is also the case for Minions and Creeps hit.
- It can damage turrets once if cast inside their range but the damage dealt is based on the skill's base damage rather than Badang's stats.
...
- Units hit by the projectiles right at the very edge only take damage but are not knocked back, stunned, nor inflicted with explosion damage.

# Trivia

- He is based on Badang, a legendary strongman in Malay legend (the hero's background differs by region and language).
- Badang is the second hero in the game that can manipulate the map's layout, being able to create walls with his abilities.
- His Susanoo skin is based on Susanoo-no-Mikoto, a kami associated with sea, storms, field, harvest, marriage and love in Japanese mythology.
