...
If Lesley doesn't take damage for 5 seconds, her next Basic Attack gains extra range and 50% Critical Chance and deals (20% extra damage against minions). Each point of fixed Physical Penetration Lesley acquires is converted into 0.5% Critical Damage, but her base Critical Damage is reduced to 140%. Lesley's Basic Attack restores 5 points of energy and her doubles the amount of energy restored. The cooldown of resets each time Lesley casts a skill.
...
Notes:
- Instead, inflicts physical damage on turrets.
...
Lesley enters the Camouflage state, gaining double Energy Regen, 40% extra Movement Speed, and 85–135 extra Physical ATK. The state lasts 3 seconds or until Lesley takes or deals damage. Enemies can detect Lesley's position through distortions of their surroundings.
...
Notes:
- (Physical ATK) starts after Master of Camouflage ends.

## Tactical Grenade

Lesley throws a tactical grenade in the target direction while slightly jumping back, dealing physical damage to enemies in a cone and knocking them back. Lesley can cast this skill during the Ultimate Snipe to cancel the state and immediately fire a Fatal Bullet at the target. The attack is only triggered when there are unfired Fatal Bullets left.
...
Lesley gains vision to nearby enemies and can lock onto the target enemy hero and fires 4 Lethal Bullets in succession, each dealing scaling damage based on level and stats and restoring 10 energy on hit. The Bullets can be blocked by other enemy heroes. If this skill is interrupted, each remaining Bullet reduces the skill cooldown by 10%. Lesley's Crit Chance is increased by 5%–15%.

Crit Chance: 5% / 10% / 15%

Base Damage: 250 / 300 / 350

Cooldown Reduction on Interrupt: (10%, 20%, 30%, 40%)
...
- If the skill is canceled without shooting any, it is not revealed to the enemy.
- The aim range is 11 unit, and max range is 15 unit.
  - The indicator is shown on the minimap when holding the skill, and the skill does not activate if there are no enemy hero within range.
...
- Both Lesley and 's royal house "Vance" means "Marsh, Mud, and Fen" in Old English which is used as a masculine given name and surname mostly of American usage.
- Lesley's Quote; *"Enemy in range, and you are too."* is a reference from Murphy's Law of Combat.
- Lesley is the first and, so far, the only Marksman/Assassin hero in the game, whereas is an Assassin/Marksman.
