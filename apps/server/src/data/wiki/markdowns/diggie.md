# Diggie

Infobox
- Image: Diggie comic.jpg
- Alias:
  - Timekeeper
  - Intellectual Owl
- Origin: Eruditio, Agelta Drylands (Eruditio, Agelta Drylands)
- Age: 15

# Story

> "The owl scholar who can control time"

Diggie is a scholar from Eruditio (Eruditio). With his "Time Nest" Diggie can turn back time and even take others to a time travel with him unhindered by any outside forces.

Gallery (image filenames only)
- Diggie_Lethal_Nexus_Map_Profile.png
- Diggie_Lethal_Nexus_Map_Background_Story.png
- Nature_Spirit
- Advisor_of_the_Eruditio_Rangers
- **Eruditio_Rangers
- *[[Diggie#Skill_1
- *Time_manipulation
- *[[Diggie#Ultimate
- *{{hi
- Diggie's_info_card.webp
- (many other gallery entries in the source, omitted here)

> "The owl scholar who can control time"

# Lore

Scholars and thinkers from all over the world are known to finally gather at the city of Antoinerei (Antoinerei). It is here where the commonly overlooked Diggie lives. Many don't five a seconds to look to Diggie due to his small frame, until they come upon his researches in chronoscience. Diggie loves clocks, he believes that within these little mechanical toys lies many more yet to be discovered secrets.

Through, the use of his "Time Nest," Diggie can control the flow of time. He is also has also able to release his friends from the physical shackles of time, taking them along with him. Beware though, for itf you anger him, be prepared for the taste of a Hoot Hoot "Time Bomb!"

# Lethal Nexus

> "Although the little owl Diggie is small in stature, it is the wise of the Eruditio Rangers. When the entire Eruditio was thrown into crisis, Diggie not only served as a mastermind, but also waved his small wings to pass information. Thanks to him, Layla and the others finally reunited successfully."

Note: Eruditio's lore has been revamped on July 2023. (outdated as of 11 December 2023)

# Notes

(no additional notes provided)

# Abilities

## Passive — Young Again
- Type: BUFF
- Terms: Untargetable; Invincible
- Description: Upon death, Diggie reverses time and turns to egg form, in which he can continue to move around and gains a new set of skills. He cannot be targeted in egg form and will revive after a period of time.
- Notes:
  - Diggie provides vision on egg form.
  - Diggie will receive 75 extra movement speed during egg form.
  - Item unique passives are disabled on egg form.
  - There will be a 2-second delay to fully control Diggie after death or revival.
    - Diggie will be vulnerable to attacks 1 second after reviving.
- Bugs:
  - Guardian Helmet Unique Passive-Recovery works during egg form.

## Skill 1 — Disdain (and Auto Alarm Bomb)
- Name: Disdain
- Skill effect: Damage
- Cooldown: 5.0
- Description: Diggie displays serious disdain for the enemy, dealing a small amount of damage to surrounding enemies while venting anger.
- Notes: deals and can deal up to 6 times.

Auto Alarm Bomb (additional info)
- Name: Auto Alarm Bomb
- Skill effects: AoE; Slowed
- Cooldown: 8.0 / 7.4 / 6.8 / 6.2 / 5.6 / 5.0
- Mana cost: 70 / 80 / 90 / 100 / 110 / 120
- Description: Diggie flings an owl alarm to the target location. The alarm will then remain stationary for 25 seconds or until it reacts to the first enemy in range, chasing them and exploding upon collision. The explosion deals base damage to nearby enemies and slows them by 30%. Diggie gains 2 stacks of Dangerous Sparks each time an enemy hero is hit by the explosion (up to 60 stacks), each stack increasing the skill's damage by 1%. Diggie loses half the stacks upon death, but can gain 1 stack of Dangerous Sparks each time his skill hits an enemy hero when in the egg form. Up to 5 owl alarms can exist at the same time.
- Properties:
  - Spell Vamp Ratio: 50%
  - Base Damage: 500 / 520 / 540 / 560 / 580 / 600
  - Extra Skill Damage Per Dangerous Sparks Stack: 1% / 2% / 3% / … / 60%
- Notes:
  - Stacking up Dangerous Sparks for each egg form skills has 1 second cooldown.
  - When thrown into an enemy unit, the owl alarm detonates immediately rather than chasing the enemy as a ward.
- Calculation formula (from source):
  ```
  total skill DMG = [(480 + 20 × skill lv.) + 100% magic power] × [100% + (1% × no. of Dangerous Sparks stacks)]
  where 0 ≤ no. of Dangerous Sparks stacks ≤ 60
  ```

## Skill 2 — Reverse Time (and Egg Dance)
- Reverse Time
  - Skill effects: CC; Damage
  - Cooldown: 15.0 / 14.0 / 13.0 / 12.0 / 11.0 / 10.0
  - Mana cost: 90
  - Description: Diggie marks a target enemy hero for 4 seconds and pulls them back to their previous location after the duration ends, dealing and slowing it by 80%. If the enemy moves out of the circle, the pull will be triggered easily.
  - Properties:
    - Base Damage: 150 / 200 / 250 / 300 / 350 / 400
  - Terms: Magic Skill; Movement Speed
- Egg Dance
  - Skill effect: Damage
  - Cooldown: 5.0
  - Description: Diggie happily dance his hometown jig, dealing a small amount of damage to enemies.
  - Notes: deals and can deal up to 10 times.

## Ultimate — Time Journey (and Victory Prayer)
- Time Journey
  - Skill effects: CC Immune; Speed Up
  - Cooldown: 76.0 / 70.0 / 64.0
  - Mana cost: 130 / 150 / 170
  - Description: Diggie removes all debuffs on nearby allied heroes (including himself) and grants them Control Immunity for 3 seconds. He also gains 50% Movement Speed for 0.5 seconds.
  - Properties:
    - Base Shield: 650 / 765 / 880
  - Terms: Control Immunity; Shield
- Victory Prayer
  - Type: BUFF
  - Cooldown: 5.0
  - Description: Diggie rolls on the ground, praying for the enemy's base to explode immediately. They say it's impossible to happen.
  - Notes: deals and can deal up to 12 times.

# Sprites

Gallery (image filenames only)
- Diggie_Outlaw_sprite.png

# Trivia

- He,, , and  are the only anthropomorphous heroes in the game.
- His design is inspired by owls, symbolizing wisdom and intelligence.
- Diggie held the record for longest gap between hero release and new skin at almost 18 months (November 2017 - May 2019), but this record was later broken by, who has not had a new skin ever since he was released in January 2019 (Cerulean Wind) and was offcialy announched in January 2021 (Kannagi).
- Diggie is the only hero where he can still be controlled by the player, even upon death.
- His in-game file name is "digger".
- Although Diggie description says it is a male, he is voiced by a female voice actor, Feena Glynn.
- His voicelines "I have a clumsy new apprentice called Einstein" is a reference to a famous scientist named Albert Einstein (https://www.britannica.com/biography/Albert-Einstein).
- He has voicelines that break the fourth wall.