{{Tabs data}}
{{Infobox data}}
{{data stats}}
== Story ==
{{Infobox data story
| name            = 
| image           = 
| full_name       = Princess Pharsa of Crow Clan
| chinese_name    = 法莎 (Fǎshā)
| alias           = 
| born            = 
| origin          = Askati Forest 
| age             =52 
| birthday        = {{birthday|1|14}}<ref name="CNProfile">Official Profile from MLBB CN</ref>
| species         = Human
| gender          = Female
| title           = 
| occupation      = 
| fields_of_study = 
| affiliation     = 
| fighting_style  = 
| weapons         = Magic staff
| abilities       = [[#Special|Disintegrates into a mist and wraps around Verri for quick travel]]
| religion        =
| ethnicity       = Crow people
| nationality     = 
| battles_fought  = 
| height          =197cm
| weight          =
| traits          = 
| likes           =
| dislikes        = 
| relationships   = *Osana (ancestor)
*Verri (spouse)
*{{hi|name=Alice}} (enemy)
| en_va           = 
| ja_va           = 
| id_va           = 
| tl_va           = 
| ar_va           = 
| pt_va           = 
| ru_va           = 
| tk_va           = 
| es_va           = 
| zh_va           = 
}}
{{Infobox character
| name            = Verri
| image           = Unused Verri icon.png
| full_name       = 
| alias           = 
| born            = 
| origin          = 
| age             = 
| species         = Human
| gender          = Male
| title           = 
| occupation      = 
| fields_of_study = 
| affiliation     = Crow clan
| fighting_style  = 
| weapons         = 
| abilities       = Shapeshifts into a crow
| religion        =
| ethnicity       = 
| nationality     = 
| battles_fought  = 
| height          = 
| weight          = 
| traits          = 
| likes           = 
| dislikes        = 
| relationships   = *Pharsa (Spouse)
*{{hi|name=Alice}} (enemy)
| en_va           = 
| ja_va           = 
| id_va           = 
| tl_va           = 
| ar_va           = 
| pt_va           = 
| ru_va           = 
| tk_va           = 
| es_va           = 
| zh_va           = 
}}
{{quote|Crow Princess of Askati Forest}}
As princess of the Crow Clan of Askati Forest, she was born with mutated blood. The mutation could gain her amazing abilities if she is awakened. When Alice invaded Askati Forest, the awakened Pharsa drove Alice away and lost her eyesight. After that, her lover turned himself into a crow to replace her eyes.
=== Bio ===
The Crow people of [[Askati Forest]] are one of the oldest races to still exist in the [[Land of Dawn]]. This race was united by the Crow King, Osana, under whose venerable leadership they perfected the art of Sky Magic. This unique school of magic allows its wielder to transform into a bird, soar across the skies, and grants the ability to strike from enormous distances, however, only those who have had their blood "awakened" can use it.

The noble Princess Pharsa was born with an unknown type of blood which was said to hold enormous potential if awakened, but as hard as they tried, the baffled people of her tribe failed to unleash its true power. Soon enough, Pharsa’s special blood became a burden, as ill-intentioned opportunists from across the land sought to reap it for their own benefit. On the day of Pharsa’s wedding, the tyrannical witch, [[Alice]] invaded the Askati Forest, mercilessly slaying countless Crows and leaving a smoldering trail of ruin in her wake. Numbed to the core by the carnage surrounding her, Pharsa dropped Osana's ancient crown to the floor. Her appearance began to change, as unprecedented power coursed through her veins—her blood had been awakened. Pharsa’s eyes clouded over, as one side of her hair turned pure white, and she soared into the sky to unleash a torrential assault of horrifying magical power, fierce enough to drive Alice away.

Verri, the love of her life, couldn’t bear to see his betrothed suffer a life of blindness alone, and chose to accompany Pharsa wherever she went, utilizing his own magic to become permanently linked to Pharsa, and allowing her to see through his eyes. Pharsa wishes her power had never been awakened, she wishes even more that her home had not been destroyed, but that witch took everything from her, and now it was time for revenge...
{{clr}}

== Abilities ==
=== Passive ===
{{Ability
|name            = Spiritual Unity
|image-2         = Spiritual Unity Buff
|skill-effect-1  = Damage
|skill-effect-2  = Slow
|quote           = 
|description     = Every 8 seconds, Verri enters the Hunting state and assists Pharsa on her next attack against an enemy data, dealing 8% extra {{B|Magic Damage}} equal to the target's {{B|Max HP}} (+1% for every 200 {{B|Magic Power}} owned) and [[slowing]] them by 60% for 1 second.
|term-1          = Magic Damage Skill
|term-2          = Movement Speed
|term-3          = 
|atk-effects     = 
|lifesteal-ratio = 
|spellvamp-ratio = 
|cooldown        = 8.0
|recharging-time = 
|mana-cost       = 
|energy-cost     = 
|duration        =
|cast-range      =
|level-scaling   = 
|stack-scaling   = 
|calc            = 
|notes           =*If {{hi|Pharsa}} uses {{ai|Wings by Wings}} immediately when Verri is flying towards the target, neither damage nor effect is applied to the target while the passive still goes on cooldown.
*Verri is also unsighted if {{hi|Pharsa}} is [[concealed]]. 
|bugs            = 
|variations-1    = Neobeast 2
}}

=== Skill 1 ===
{{Ability
|name            = Curse of Crow
|skill-effect-1  = CC
|skill-effect-2  = Damage
|quote           = 
|description     = Pharsa unleashes a blast of magic at the target location, dealing {{scale|base=300-425|total-mp=120|md}} to enemies hit and marking them for 4 seconds. <br> Pharsa's subsequent attack against marked enemies will consume the mark and [[stun]] them for 1 second.
|term-1          = Stun
|term-2          = Magic Damage Skill
|term-3          = 
|atk-effects     = 
|lifesteal-ratio = 
|spellvamp-ratio = 50%
|cooldown        = 8.0 / 7.6 /7.2/ 6.8 /6.4 / 6.0
|recharging-time = 
|mana-cost       = 60 / 66 / 72 / 78.0 / 84 / 90
|energy-cost     = 
|duration        =
|cast-range      =
|level-scaling   = Base Damage: 300 / 325 / 350 / 375 /400/ 425
|stack-scaling   = 
|calc            = 
|notes           =*Affected enemies are revealed.
*The ''subsequent attack'' in the description only consider the following:
**{{hi|Pharsa}}'s basic attack
**{{ai|Spiritual Unity}}
**{{ai|Energy Impact}}
**{{ai|Feathered Air Strike}}
*Being hit again by the ability reapplies the debuff. 
|bugs            = 
|variations-1    = Neobeast 2
}}

=== Skill 2 ===
{{Ability
|name            = Energy Impact
|skill-effect-1  = AOE
|skill-effect-2  = 
|quote           = 
|description     = Pharsa releases magic energy in the target direction, dealing {{scale|base=525-700|total-mp=145|md}} to enemies hit.
|term-1          = Magic Damage Skill
|term-2          = 
|term-3          = 
|atk-effects     = 
|lifesteal-ratio = 
|spellvamp-ratio = 50%
|cooldown        = 5.0
|recharging-time = 
|mana-cost       = 50 / 56 / 62 / 68 / 74 / 80
|energy-cost     = 
|duration        =
|cast-range      =
|level-scaling   = Base Damage: 525 / 560 / 595 / 630 / 665 / 700
|stack-scaling   = 
|calc            = 
|notes           = 
|bugs            = 
|variations-1    = Neobeast 2
}}

=== Ultimate ===
{{Ability
|image           = Feathered Air Strike
|image-2         = Feathered Air Strike 2
|image-3         = 
|image-4         = 
|image-5         = 
|image-6         = 
|name            = Feathered Air Strike
|skill-effect-1  = Burst
|skill-effect-2  = 
|quote           = 
|description     = Pharsa takes flight for 8 seconds and bombards the target location, dealing {{scale|570-870|total-mp=160|md}} to enemies within the area. This skill can be cast up to 4 times within the duration.
|term-1          = Magic Damage Skill
|term-2          = 
|term-3          = 
|atk-effects     = 
|lifesteal-ratio = 
|spellvamp-ratio = 50%
|cooldown        = 36.0 / 33.0 / 30.0
|recharging-time = 
|mana-cost       = 120 / 150 / 180
|energy-cost     = 
|duration        = 8
|cast-range      =
|level-scaling   = Base Damage: 570 / 720 / 870
|stack-scaling   = 
|calc            = 
|notes           =*The ability has an estimate range of 9.5 and hitbox of 1.9.
*{{hi|Pharsa}} can cast any other abilities, [[battle spells]] and [[equipment]]s, except for {{ai|Wings by Wings}} and {{ai|Recall}} without canceling this ability.
*The casting of the ability is also the first cast. 
|bugs            = *On Pharsa's official data spotlight at www.mobilelgends.com, it erroneously categorized Energy Impact as "3rd Skill."
|variations-1    = Neobeast 2
}}

=== Special ===
{{Ability
|image           = Wings by Wings
|image-2         = Wings by Wings 2
|image-3         = 
|image-4         = 
|image-5         = 
|image-6         = 
|name            = Wings by Wings
|skill-effect-1  = Morph
|skill-effect-2  = Speed Up
|quote           = 
|description     = Pharsa turns into mist and wraps herself around Verri, gaining 150% extra [[Movement Speed]] (decaying over time) and the ability to fly over terrain. This state lasts up to 5 seconds or until she uses a {{ii|Basic Attack}} or skills, or is controlled.
|term-1          = Ignore Obstacles
|term-2          = Movement Speed
|term-3          = 
|atk-effects     = 
|lifesteal-ratio = 
|spellvamp-ratio = 
|cooldown        = 23.0
|recharging-time = 
|mana-cost       = 
|energy-cost     = 
|duration        = 5
|cast-range      =
|level-scaling   = 
|stack-scaling   = 
|calc            = 
|notes           = *The camera height raises within the skill channeling.
|bugs            = *On Pharsa's official data spotlight at www.mobilelgends.com, it erroneously categorized Wings by Wings as an "Ultimate".
|variations-1    = Neobeast 2
}}

== Gallery ==
=== Splash art ===
<gallery captionalign="center" spacing="small" hideaddbutton="true">
Pharsa (Wings of Vengeance).jpg|Wings of Vengeance
Pharsa (Wings of Heaven).jpg|Wings of Heaven
Pharsa (Peafowl's Dance).jpg|Peafowl's Dance
Pharsa (Indigo Aviatrix).jpg|Indigo Aviatrix
Pharsa (Enchanting Witch).jpg|Enchanting Witch
Pharsa (Empress Phoenix).jpg|Empress Phoenix
Pharsa (Samba Muse).jpg|Samba Muse
Pharsa (Peony Bloom).jpg|Peony Bloom
Pharsa (Hierophant).jpg|Hierophant
Pharsa (Neon Shingetsu).jpg|Neon Shingetsu
Pharsa (Neobeast Pharsa).jpg|Neobeast Pharsa
</gallery>
=== Avatar icons ===
<gallery bordercolor="transparent" captionalign="center" spacing="small" widths="100" hideaddbutton="true">
Hero521-icon.png|Wings of Vengeance
Hero522-icon.png|Wings of Heaven
Hero523-icon.png|Peafowl's Dance
Hero524-icon.png|Indigo Aviatrix
Hero525-icon.png|Enchanting Witch
Hero526-icon.png|Empress Phoenix
Hero527-icon.png|Samba Muse
Hero528-icon.png|Peony Bloom
Hero529-icon.png|Hierophant
Hero105210-icon.png|Neon Shingetsu
Hero105211-icon.png|Neobeast Pharsa
</gallery>
===Battle emotes===
<gallery bordercolor="transparent" captionalign="center" spacing="small" widths="100" hideaddbutton="true">
Almost!.png|'''Almost!'''<br>Obtained from Emote Bundle.
Battle Emote - Awesome.png|'''Awesome'''<br>Obtained via the Gallery after collecting the Neobeasts Phase II skins.
</gallery>

==Videos==
<gallery captionalign="center" spacing="small" hideaddbutton="true">
Mobile Legends- Bang Bang! New Hero -Wings of Vengance- Pharsa|{{hi|name=Pharsa}} Hero Spotlight<br>'''NOTE''': This Hero Spotlight Video is before revamped/reworked. {{Ability Icon|name=Wings by Wings|Wings by Wings (First Skill)}} shifed to Utility Skill replacing by new first skill {{Ability Icon|name=Curse of Crow}}.
</gallery>

== Trivia ==
* The name Pharsa could be a reference to ancient '''Egyptian royalty''', as the word '''"Pharaoh"''' has connotations of '''power''', '''majesty''', and '''divine authority'''. This could reflect Pharsa’s '''dominant presence''' in battle and her '''command over flight and magical energy'''.
* '''"Pharsa"''' could be inspired by the Arabic word for '''"bird"''' or '''"eagle,"''' which fits Pharsa's '''avian''' and '''flight-based abilities'''. As a '''mage data''' with a powerful bird companion and the ability to soar through the air, this interpretation aligns well with her design and skillset.
* Her name could be also based on Persian word, Parsa meaning pious or devout.
* Pharsa's character might be inspired by the Irish goddess of Vengeance, [[wikipedia:The Morrígan|Morrìgàn]], who also often takes the shape of a Crow and is portrayed wearing a feathered cape.
* As princess of the Crow Clan of Askati Forest, she was born with mutated blood.
*Prior to her revamp, she was temporarily banned in [[Brawl|Brawl games]] as her  ultimate skill gave a huge disadvantage to her enemies due to its short cooldown, range, and damage.

== References ==
{{reflist}}

== Navigation ==
{{heroes}}
