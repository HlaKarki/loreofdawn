{{Hero navigation}}
{{Infobox hero}}
{{Hero stats}}
== Story ==
{{Translation|Birthday (生日 "shēngrì" meaning "birthday"), Official Chinese Name}}
{{Retcon|{{Slink|Alpha|Story}}}}
{{Infobox hero story
| full_name      = Peace Android Prototype 1
| alias          = Bunnylove
| chinese_name   = 安卓儿 (ānzhuóer, "android")
| birthday       = {{birthday|3|13}}<ref name="AngelaCNProfile">[https://weibo.com/7711876938/P4d13s0fg Official profile from Weibo]
<gallery>
Profile CN Angela.jpg
</gallery>
</ref>
| born           = [[Laboratory 1718]], [[Baker Island]]
| origin         = 
| species        = Android
| gender         = Female
| title          = 
| occupation     = 
| affiliation    = [[Eruditio]]
| fighting_style = 
| weapons        = *Angela's Doll
*[[Angela#Skill|Puppet string]]
| abilities      = *[[Angela#Passive|Smart Heart]]
*Releases [[Angela#Skill 1|Love Energy]]
*[[Angela#Ultimate|Attaching to an ally]]
| battles_fought = 
| height         = 
| weight         = 
| traits         = 
| likes          = 
| dislikes       = 
| relationships  = *Dr. Baker (creator)
*{{Hi|Alpha}} (enemy)
*{{Hi|Saber}} (ally)
*{{Hi|Chip}} (confidant, adoptive sibling)
| en_va=
| id_va= Uci Bismark{{cite}}
| ja_va= Ueda Hikaru (植田 ひかる)<ref name="In-game credit">In-game credit</ref>
| tl_va=
| ar_va=
| pt_va=
| ru_va=
| tk_va=
| es_va=
| zh_va=
}}
{{Quote|Artificial human made in Laboratory 1718}}
An android created by the founder of [[Laboratory 1718]], [[Dr. Baker]]. It was the first life form created without using human body or soul. In order to gain the knowledge of her, other scientists in the lab went after Dr. Baker. Before taken away, Dr. Baker stuffed her into a flight capsule to escape.
=== Lore ===
Dr. Baker was one of the co-founders of Laboratory 1718, an institution created with the aim of bringing peace to the world. But as his colleagues, that is, the other scientists fell deeper and deeper into their depraved human weaponization experiments, Dr. Baker felt he had no choice but to leave all this behind and pursue a new project. Renouncing the questionable ethics of human experimentation, he endeavored to create a new kind of lifeform, solely from a mechanical skeleton, puppet strings, and synthetic skin. Finally, a new generation of artificial life was born. Dr. Baker firmly believed that, love and hope were the greatest inventions of mankind. So, he programmed the world's most moving stories into the heart of his new creation, transforming her into an android-based robot, meant for helping others.

One bright morning, Peace Android Prototype 1 awakened from her bed, and Dr. Baker bestowed upon this adorable mechanical angel, the Name Angela. Angela followed Dr. Baker's tutelage, studying hard to understand the world, and hoping to meet the inventor's high expectations. However, their peaceful life together, was soon cut short. When the mad scientist of Lab 1718, heard rumors that Dr. Baker had created a living soul outside a human body, which didn't meet their ideas of creating flesh weapons, they couldn't bear to believe it. They were convinced among themselves that Baker had betrayed them, keeping this knowledge to himself before fleeing, and so they ordered Dr. Baker's past creations, {{hi|Alpha|Alpha}} and Beta to hunt down Dr. Baker and his creation. As their pursuers were just about to close in on them, Dr. Baker placed Angela into a flight capsule, instructing her to seek out his good friend [[Dr. Rooney|Rooney]].
=== Artificial Soul — The Creation of Angela ===
[[File:Artificial Soul - The Creation of Angela.png|right|x512px]]
Dr. Baker was one of the founders of Lab 1718, an institution created with the aim of bringing peace to the world. But as his co-founders fell deeper and deeper into their depraved human weaponization experiments, Dr. Baker felt he had no choice but to leave and pursue a new project.

Renouncing the questionable ethics of human experimentation, he endeavored to create a new kind of life-form, solely from a mechanical skeleton, puppet strings, and synthetic skin, and finally, a new generation of artificial life was born. Dr. Baker firmly believed that love and hope were the greatest inventions of mankind, so he programmed the world's most moving stories into the heart of his new creation. One bright morning, Peace Android Prototype 1 awakened from her bed, and Dr. Baker bestowed upon this adorable mechanical angel, the name Angela.

Angela followed Dr. Baker's tutelage, studying hard to understand the world, and hoping to meet the doctor's high expectations. However, their peaceful life together was soon cut short. When the mad scientists of Lab 1718 heard rumors that Dr. Baker had created a living soul outside of a human body, they couldn't believe it. They convinced themselves that Baker had betrayed them, keeping this knowledge to himself before fleeing, and so they ordered Dr. Baker's past creations, Alpha and Beta to hunt down Dr. Baker and his creation. As their pursuers were just about to close in on them, Dr. Baker placed Angela into a flight capsule, instructing her to seek out his good friend Dr. Rooney. However, the flight capsule was struck down a terrible thunderstorm, and landed in a remote corner of the Land of Dawn. Emerging from the wreckage, Angela began her journey to find Dr. Rooney, and save her father.
{{clr}}

== Abilities ==
=== Passive ===
{{Ability
|variations-1   = Sanrio
|variations-2   = The Aspirants
|variations-3   = Kishin Densetsu
|name           = Smart Heart
|skill-effect-1 = Speed Up
|skill-effect-2 = 
|cooldown       = 
|mana-cost      = 
|energy-cost    = 
|quote          = 
|description    = Angela gains 7.5% extra [[Movement Speed]] for 4 seconds each time she casts a skill (up to 30%). When she's attached to an allied hero, the Movement Speed boost is transferred to the allied hero.
|properties=*'''Extra [[Movement Speed]] Per Stack:''' 7.5%/ 15% / 22.5% / 30%
|calc           = 
|term-1         = Movement Speed
|term-2         = 
|term-3         = 
|notes          = 
|bugs           = 
}}
=== Skill 1 ===
{{Ability
|variations-1   = Sanrio
|variations-2   = The Aspirants
|variations-3   = Kishin Densetsu
|image-2        = 
|image-3        = 
|image-4        = 
|image-5        = 
|image-6        = 
|name           = Love Waves 
|skill-effect-1 = HEAL
|skill-effect-2 = AOE
|spellvamp-ratio= 50%
|cooldown       = 2.0
|recharging-time= 6.0
|mana-cost      = 60 / 70 / 80 / 90 / 100 / 110
|energy-cost    = 
|quote          = 
|description    = Angela sends {{B|Love Waves|e}} in the target direction, dealing {{Scale|base=170-320|total-mp=80|md}} to enemies hit, [[revealing]] their positions briefly,<sup>[''duration needed'']</sup> and applying a stack of {{b|Lover's Mark|mark}} to them (up to 5 stacks). Each stack of Lover's Mark increases the damage by 20% and [[slows]] enemies hit by 8% for 3 seconds. <br> The {{B|Love Waves|e}} also restore {{Scale|base=150-200|total-mp=75|hp}} to allied heroes hit. Angela can store up to 5 charges of this skill and the recharge time is affected by [[Cooldown Reduction]].
|stack-scaling  = Damage Increase Per Lover's Mark Stacks: 20% / 40% / 60% / 80% / 100%
Slow Effect Per Lover's Mark Stacks: 8% / 16% / 24% / 32% / 40%
|level-scaling= Base Damage: 170 / 200 / 230 / 260 / 290 / 320
Healing Bonus: 150 / 160 / 170 / 180 / 190 / 200
|ss-3           = 
|calc           = <math>{\rm total\ skill\ DMG} = \bigl[ 140 + (30 \times skill\ lv.) + 80%\ total\ magic\ power \bigr] + (20% \times no.\ of\ Lover\text{'}s\ Mark\ stacks)</math>
|term-1         = Skill Charging Time
|term-2         = Magic Damage Skill
|term-3         = Healing Effect
|notes          = *With Assisted Aiming Mode enabled, Angela can cast Love Waves at nearby allied heroes if there's no enemy around her. (Added on [[Patch Notes 1.7.94]].)
*Lover's Mark cannot be applied to [[minion]]s.
|bugs           = 
}}

=== Skill 2 ===
{{Ability
|variations-1   = Sanrio
|variations-2   = The Aspirants
|variations-3   = Kishin Densetsu
|name           = Puppet-on-a-String
|skill-effect-1 = Slow
|skill-effect-2 = CC
|cooldown       = 13.0 / 12.4 / 11.8 / 11.2 / 10.6 / 10.0
|mana-cost      = 90 / 100 / 110 / 120 / 130 / 140
|energy-cost    = 
|quote          = 
|description    = Angela launches a {{b|Puppet String|e}} at the target enemy, dealing {{scale|base=300-450|total-mp=40|md}} and gradually [[slowing]] them up to 80%. If the enemy is still connected to the string after 3 seconds, they'll be [[immobilized]] for 1.5 seconds and dealt {{Scale|base=450-675|total-mp=60|md}}. <br> Each stack of {{b|Lover's Mark|mark}} increases the final damage by 20%.
|level-scaling= 
Base Damage: 300 / 330 / 360 / 390 / 420 / 450
Extra Damage: 450 / 495 / 540 / 585 / 630 / 675
|ls-3           = 
|ls-4           = 
|ss-1           = 
|ss-2           = 
|ss-3           = 
|calc           = <math>{\rm final\ skill\ DMG} = \bigl[ 405 + (45 \times skill\ lv.) + 60%\ total\ magic\ power \bigr] + (20% \times no.\ of\ Lover\text{'}s\ Mark\ stacks)</math><br><math>{\rm total\ movement\ SPD} = initial\ Movement\ SPD \times \left( 100 - \frac {4 \times skill\ duration} {15} \right)</math>
|term-1         = Immobilize
|term-2         = Movement Speed
|term-3         = Magic Damage Skill
|notes          = 
|bugs           = *Slow effect from this skill cannot be interrupted by slaying Angela.
}}

=== Ultimate ===
{{Ability
|variations-1   = Sanrio
|variations-2   = The Aspirants
|variations-3   = Kishin Densetsu
|name           = Heartguard
|image-2        = Heartguard 2
|image-3        = Heartguard 3
|skill-effect-1 = Attach
|skill-effect-2 = Shield
|cooldown       = 70.0
|mana-cost      = 100 / 150 / 200
|energy-cost    = 
|quote          = 
|description    = Angela begins [[channeling]] [for 2 seconds], applying a {{Scale|base=1000-2000|total-mp=200-300|shield}} that lasts 6 seconds to the target allied hero regardless of distance. After the channeling is complete, she attaches herself to the allied hero for 12 seconds, during which she can cast skills at zero [[Mana]] cost. <br> Angela can cast this skill again to cancel the channeling or detach herself from the allied hero. The attaching state will end early if the allied hero is killed.
|level-scaling  =
Cast Range: infinite
Base [[Shield]]: 1000 / 1300 / 1600
Shield Bonus: 200% / 250% / 300%
|calc           = 
|term-1         = Shield 
|term-2         = 
|term-3         = 
|notes          = *During the attaching state, Angela can cast this skill again to detach herself from the allied hero by [[Blink|blinking]] to the designated location away from the attached hero.
*[[Cooldown]] starts after casting the skill.
*The channeling phase can be interrupted by [[crowd control]] (that can interrupt skills) and [[displacement]] effects.
*Angela is [[untargetable]] during the Attaching State and cannot use any [[equipments]] or [[battle spells]]
*The effect persists if target ally becomes untargetable, such as Ling, Lunox or Hanzo.
*Cannot cast on ally inside {{hi|Yin}}'s domain nor allies outside of it if she is inside the domain, nor update their current health.
**An ally being pulled into {{hi|Yin}}'s domain will force {{hi|Angela}} to pop out.
*Despite not having '''Mobility''' tag, this ability is disabled in {{hi|Minsitthar}}'s {{ai|King's Calling}}.
**{{hi|Angela}} can neither leave the attached ally when the ally is in {{hi|Minsitthar}}'s {{ai|King's Calling}}
|bugs           =
}}

== Gallery ==
=== Splash art ===
<gallery captionalign="center" spacing="small" hideaddbutton="true">
Angela (Bunnylove).jpg|Bunnylove
Angela (Dove & Love).jpg|Dove&Love
Shanghai Maiden wall.png|Shanghai Maiden
Angela (V.E.N.O.M. Vespid).jpg|V.E.N.O.M. Vespid
Angela (Scream Doll).jpg|Scream Doll
Angela (Summer Vibes).jpg|Summer Vibes
Angela (Floral Elf).jpg|Floral Elf
Angela (Heartstring).jpg|Heartstring
Angela (Cyber Cherubin).jpg|Cyber Cherubin
Angela (Avatar of Time).jpg|Avatar of Time
Angela (Strings of Fate).jpg|Strings of Fate
</gallery>
=== Avatar icons ===
<gallery captionalign="center" spacing="small" bordercolor="transparent" widths="100" hideaddbutton="true">
Hero551-icon.png|Bunnylove
Hero552-icon.png|Dove&Love
Hero553-icon.png|Shanghai Maiden
Hero554-icon.png|V.E.N.O.M. Vespid
Hero555-icon.png|Scream Doll
Hero556-icon.png|Summer Vibes
Hero557-icon.png|Floral Elf
Hero558-icon.png|Heartstring
Hero559-icon.png|Cyber Cherubin
Hero105510-icon.png|Avatar of Time
Hero105511-icon.png|Strings of Fate
</gallery>
===Battle emotes===
<gallery captionalign="center" spacing="small" bordercolor="transparent" widths="100" hideaddbutton="true">
Angela (Sweet Summer) - Battle Emote.png|'''Sweet Summer'''<br>Obtained from Emote Bundle.
Angela (Marionette) - Battle Emote.png|'''Marionette'''<br>Obtained from Emote Bundle.
Angela (Happy Ramadan) - Battle Emote.png|'''Happy Ramadan'''<br>Obtained via the Ramadan event.
Angela (Let's Go, Tetra!) - Battle Emote.png|'''Let's Go, Tetra!'''<br>Obtained via the Aspirants Unite limited-time events.
Battle Emote - Support Cheer.png|'''Support Cheer'''<br>Obtained via the 2023 [[Starlight|StarLight Fest]] events.
Battle Emote - Looking Forward.png|'''Looking Forward'''<br>Obtained via the [[Kishin Densetsu]] events.
</gallery>
=== Videos ===
<gallery captionalign="center" spacing="small" hideaddbutton="true">
Mobile Legends Bang Bang! New Hero Bunnylove Angela Gameplay
</gallery>

==Trivia==
*She is the first hero who can be attached to a hero by using her Ultimate, the second is {{hi|Gloo}}.
*She carries a distinct doll for each skin that resembles her appearance on the hero selection screen. She used to toss the puppet as her basic attack when she was still in development, but the animation was eventually altered to simple heart-shaped projectiles. This doll can now only be visible in-game when she is using her {{ai|Heartguard}} to possess an ally. The doll will be seen above the ally hero's model.
*Because some users found her former eyes unsettling and distressing, Angela's on her splash art was modified.
*Her [[StarLight Member|Starlight]] skin, was renamed from China Doll to Shanghai Maiden.

== References ==
{{reflist}}
{{Heroes}}