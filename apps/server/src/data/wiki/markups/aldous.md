{{Hero navigation|Main}}
{{Infobox hero}}
{{Hero stats}}
== Story ==
{{Translation|Official chinese name}}
{{Infobox hero story
|image=<gallery>
Aldous comic.jpg|Present
Aldous old.jpg|Before the divine punishment
</gallery>
| full_name      = 
| chinese_name   = 毅拳 (yìquán, "Decisive Fist"<ref>毅 (yì) meaning "resolute, decisive, firm, staunch"<br>拳 (quán) meaning "fist, chinese boxing"</ref>)
| alias          = *The Contractor
| born           = 
| birthday       = {{birthday|1|18}}<ref name="CNProfile">Official Profile from MLBB CN</ref>
| origin         = [[Minoan Maze]], [[Agelta Drylands]]
| species        = DemiGod
| gender         = Male
| title          = 
| occupation     = Mercenary
| affiliation    = *Minoan Maze
*[[The Abyss]] (enemies)
| fighting_style = 
| weapons        = 
| abilities      = 
| battles_fought = 
| height         = 
| weight         = 
| traits         = 
| likes          = 
| dislikes       = 
| relationships  = *{{hi|Minotaur}} (companion)
*{{hi|Selena}} (enemy)
|en_va=[[IMDB:nm9240513|Cian Hoyne]]<ref name="cianhoyne>[https://www.youtube.com/watch?v=qZ6JdIdH1bc&ab_channel=CianHoyne Cian Hoyne]</ref>
|ja_va=[[wikipedia:Hiromu Mineta|Mineta Hiromu (峯田 大夢)]]<ref name="In-game credit">In-game credit</ref>
|id_va=Harry Suseno{{cite}}
|tl_va=
|ar_va=
|pt_va=
|ru_va=
|tk_va=
|es_va=
|zh_va=
}}
{{quote|Guardian of the Minoan Maze}}
A guard of the Minos Labyrinth, who kept his pledge even after the kingdoms fall. He has blocked off intruders who covered the kingdoms heritage for a century with an unusually huge arm.
=== Lore ===
Legends from the [[Land of Dawn]] say that the Western Desert's Minoan Kingdom possesses a [[Minoan Maze|mysterious labyrinth]]. In the maze is a statue of giant minotaur, which the people consider to be the mecca of the Minoan Kingdom. Some say that there is a group of mercenaries who guard the entire maze. The Kingdom produced a lot of precious jade, which brought the Minoan people great wealth. Very quickly, the entire kingdom was lost in the intoxicating allure of gems. Even many mercenaries, tasked with guarding the sacred maze become corrupted, abandoning their duty. Only one stayed―Aldous, who had never forgotten his duty and guard the Minoan Labyrinth.

Until one day, a god came from the heavens to punish these lands. The powers of light and dark decimated the entire Minoan Kingdom, which even Aldous could not escape. The two streams dark and light energy surged into his body. Aldous thought he was about to die. However, the two streams of energy collided, causing great changes to his body. His skin grew long-black scales that could resist most attacks and monstrously large arms that could easily strike fear into people's hearts.

When Aldous awoke from his daze, he found his people's kingdom all gone and the mysterious maze was now sealed by some unknown force. The remaining minotaurs decided that Aldous would stay to defend the maze, while the others travel across the Land of Dawn to find answers. News of a mysterious force sealing the Minoan Maze spread quickly to the Abyssal Kingdom. To gather this strange energy, [[Selena]] decided to bring her Abyssal Demons with her to the entrance of the Minoan Maze―A huge battle was about to begin.
=== Comic ===
{{Main|Scourge of the Gods}}
{{clear}}

== Abilities ==
=== Passive ===
{{Ability
|variations-1       = Transformers
|image              = Contract Transform
|name               = Contract: Transform
|skill-effect-1     = Shield
|skill-effect-2     = 
|image-transformers = Contract Transform
|skill-type-1       = 
|skill-type-2       = 
|skill-type-3       = 
|cooldown           = 5.0
|quote              = 
|description        = After every 2 {{ai|Basic Attack|attacks}}, Aldous gains a 500 (+3 × {{icon|a|Contract Soul Steal|Soul Steal}} stack count) {{AVC1|shield|Shield}} for 3 seconds on his next {{ai|Basic Attack}}.<br>This effect can only be triggered once every 5 seconds.
|ss-1               = Shield: 503 / 506 / 509 / … / 2000
|term-1             = Physical Basic Attack
|term-2             = Shield
|term-3             = 
|notes              = 
}}
=== Skill 1 ===
{{Ability
|variations-1       = Transformers
|image              = Contract Soul Steal
|name               = Contract: Soul Steal
|skill-type-1       = Physical Basic Attack
|skill-type-2       = 
|skill-type-3       = 
|cooldown           = 4.0 / 4.0 / 4.0 / 4.0 / 4.0 / 4.0
|mana-cost          = 60 / 65 / 70 / 75 / 80 / 85
|quote              = 
|description        = Aldous unleashes his inner energy to enhance his next [[Basic Attack]] to kill enemies and absorb their soul each stack gives to deal {{b|Physical Damage}} equal to {{Scale|base=200-450|total-pa=100|pd}} plus 5 times the number of Soul Steal stacks.<br>If this attack kills an <nowiki>[</nowiki>[[Lord]]/[[Turtle]]/<nowiki>]</nowiki>enemy hero/[enemy turret/][[Siege Minion]]/other-non hero unit, Aldous will gain [12/12/10/10/10/4] stacks of {{b|Soul Steal|e}} (up to 650 stacks).<br>The enhanced Basic Attack deals 300% extra Base Damage to [[Minions]] and [[Creeps]]. Aldous gets extra 5/2 stack) when a nearby enemy Siege Minion/other non-hero unit dies.
|ls-1               = Base Damage: 200 / 250 / 300 / 350 / 400 / 450
|ls-2               = 
|ls-3               = 
|ls-4               = 
|ss-1               = Extra Damage: 5 / 10 / 15 / … / 2500
|ss-2               = 
|ss-3               = 
|calc               = <math>{\rm extra\ physical\ damage} = 5 \times no.\ of\ stacks</math>
|term-1             = Physical Basic Attack
|term-2             = 
|term-3             = 
|notes              = *The enhanced basic attack lasts for 5 seconds or until it is consumed.
*Cooldown starts upon casting the skill, but the skill cannot be used again until the enhanced basic attack is consumed or expired.
*Aldous [[charges]] towards a target when attacking using his enhanced basic attack.
*The enhanced basic attack's base damage and extra damage do not [[critically strike]].
*Gain stack when dying enemies have {{ai|Immortality}} available.
}}

=== Skill 2 ===
{{Ability
|variations-1       = Transformers
|image              = Contract Explosion
|image-2            = Contract Explosion 2
|skill-effect-1     = BUFF
|skill-effect-2     = CC
|skill-type-1       = Physical Skill
|skill-type-2       = 
|skill-type-3       =
|name               = Contract: Explosion
|cooldown           = 12.0 / 11.5 / 11.0 / 10.5 / 10.0 / 9.5
|mana-cost          = 80 / 90 / 100 / 110 / 120 / 130
|quote              = 
|description        = Aldous assumes a defensive stance, gaining 30% [[Damage Reduction]] and 20% extra [[Movement Speed]] for 2 seconds or until manual cancellation. He then deals {{Scale|base=100-200|total-pa=20}}&ndash;{{Scale|base=200-400|total-pa=40|pd}} to nearby enemies and stuns them for 0.5&ndash;1 second(s) (scales with defensive stance duration).
|ls-1               = Minimum Damage: 100 / 120 / 140 / 160 / 180 / 200
|ls-2               = Maximum Damage: 200 / 240 / 280 / 320 / 360 / 400
|ls-3               = 
|ls-4               = 
|ss-1               = 
|ss-2               = 
|ss-3               = 
|calc               = <math>\rm{base\ damage} = \bigl[80 + (20 \times skill\ level) \bigr] + \bigl[(40 + 10 \times skill\ level) \times skill\ duration \bigr]</math><br><math>{\rm physical\ bonus} = 20% + (10% \times skill\ duration)</math><br><math>{\rm stun\ duration} = 0.5 s + (0.25 s \times skill\ duration)</math><br><math>{\rm where} = 0s \leq skill\ duration \leq 2s</math>
|term-2             = Damage Reduction
|term-3             = Stun
|term-1             = Dodge
|notes              = 
}}

=== Ultimate ===
{{Ability
|variations-1       = Transformers
|image              = Contract Chase Fate
|image-2            = Contract Chase Fate 2
|image-3            = Contract Chase Fate 3
|name               = Contract: Chase Fate
|skill-effect-1     = Blink
|skill-effect-2     = Damage
|skill-type-1       = 
|skill-type-2       = 
|skill-type-3       = 
|cooldown           = 60.0 / 55.0 / 50.0
|mana-cost          = 150 / 200 / 250
|quote              = 
|description        = Aldous gains [[vision]] of all enemy heroes for 5 seconds. During this time, if Aldous uses this skill again, he can [[dash]] toward an enemy hero [of choice]. When Aldous hits that enemy hero, he will deal {{scale|base=400-750|total-hp=target's 8|pd}} and [[knocked back|knock back]] the target for 1 second. (Aldous can interrupt target's {{ai|Recall}} [and {{ai|Arrival}}] during the dash.)<br>[Use this skill once again to cancel the dash, and will [[blink]] to the designated direction instead.]
|ls-1               = Base Damage: 250 / 325 / 400
|ls-2               = 
|ls-3               = 
|ls-4               = 
|ss-1               = 
|ss-2               = 
|ss-3               = 
|calc               = 
|term-1             = Displacement
|term-2             = Max HP
|term-3             = Physical Damage Skill
|notes              = *With the perfect timing, it is possible to interrupt Aldous from dashing using {{hi|Grock}}'s {{ai|Guardian's Barrier}} or {{hi|Yin}}'s {{ai|My Turn}}.
*The dash can only be interrupted by [[suppressed]].
*He can freely use [[equipment]]s and [[battle spells]].
*Inside {{hi|Yin}}'s domain, he cannot lock on enemies outside and ongoing travel is cancelled.
**Likewise, he cannot select targets inside the domain, and does not pursue further when arriving in the target's last location. 
}}

==Gallery==
=== Splash art ===
<gallery captionalign="center" spacing="small" hideaddbutton="true">
Aldous Contractor wall.png|Contractor
Red Mantle wall.png|Red Mantle
Aldous (Death).jpg|Death
Aldous (The Insentient).jpg|The Insentient
Aldous (King of Supremacy).jpg|King of Supremacy
Aldous (Blazing Force).jpg|Blazing Force
Aldous (Realm Watcher).jpg|Realm Watcher
Aldous (Starscream).jpg|Starscream
Aldous (Mistbender Aldous).jpg||Mistbender Aldous
</gallery>

===Avatar icons===
<gallery spacing="small" captionalign="center" bordercolor="transparent" widths="100" hideaddbutton="true">
Hero641-icon.png|Contractor
Hero642-icon.png|Red Mantle
Hero643-icon.png|Death
Hero644-icon.png|The Insentient
Hero645-icon.png|King of Supremacy
Hero646-icon.png|Blazing Force
Hero647-icon.png|Realm Watcher
Hero648-icon.png|Starscream
Hero649-icon.png|Mistbender Aldous
</gallery>
===Battle emotes===
<gallery spacing="small" captionalign="center" bordercolor="transparent" widths="100" hideaddbutton="true">
Escape.png|'''Escape?'''<br>Battle Emote - 2019/07 StarLight Special
Fist of Glory.png|'''Fist of Glory'''<br>Obtain via Glory Bundle
My Time will Come!.png|'''My Time will Come!'''<br>Obtained via the MLBB X Transformers event.
Battle Emote - Clenched Fist.png|'''Clenched Fist'''<br>Obtained via the Mistbenders event.
</gallery>
===Videos===
<gallery captionalign="center" spacing="small" hideaddbutton="true">
Mobile Legends Bang Bang! New Hero Contractor Aldous
Aldous New Skin Death Mobile Legends Bang Bang!
Aldous new skin The Insentient Mobile Legends Bang Bang!
Aldous New Skin King of Supremacy Mobile Legends Bang Bang
</gallery>

== Trivia ==
*The name Aldous is a boy's name of English origin meaning "old." Aldous has long been associated with British writer Huxley, author of Brave New World.
*His voice line "Abandon hope, all ye who enter here." is a direct reference to Dante's quote in [https://en.wikipedia.org/wiki/Inferno_%28Dante%29?wprov=sfla1 Inferno].
*While still in development Aldous' first given name and hero title was Armstrong, Gravekeeper. It was later changed to Aulrad, Contractor. However, in [[Patch Notes 1.2.78|Patch 1.2.78]], his name was changed again to Aldous.
*Aldous "Red Mantle" refers to his former appearance without his damaged body.
*Aldous is the sole survivor of the divine punishment that devastated the Agelta Drylands. He was blessed by the Twilight Orb, which not only saved his life but also granted him immense power, granting him strength nearly equal to that of Gods or Supreme beings.
*Aldous is one of only two heroes capable of harnessing the power of the Twilight Orb, the other being is {{hi|Lunox}}.

== References ==
{{reflist}}
== Navigation ==
{{Heroes}}
{{Hero categories}}
[[Category:2018 heroes]]