{{Tabs data}}
{{Infobox data}}
{{Hero stats}}
==Story==
{{Translation|Temperament (气质 "qìzhì" meaning "temperament, disposition, manners, aura qualities) or Traits}}
{{Infobox data story
|image          = Benedetta comic.png
|full_name      = 
|alias          = Shadow Ranger
|chinese_name   = 贝拉 (bèilā)
|born           = Rantha Mountains
|origin         = 
|age            = 
|birthday       = {{birthday|4|15}}<ref name="BenedettaCNProfile">[https://weibo.com/7711876938/5150313850077621 Official profile from Weibo]
<gallery>
Profile CN Benedetta.jpg
</gallery>
</ref>
|species        = Human
|gender         = Female
|title          = 
|occupation     = Traveler
|affiliation    = 
|fighting_style = 
|weapons        = "Alecto" katana
|abilities      = *Dashes as she draws out Alecto 
*[[#Skill 1|Summons shadow]]
|battles_fought = 
|height         = 
|weight         = 
|traits         = Firmly Unyielding, Rebelliously Grim<ref name="BenedettaCNProfile"/><ref>Original Text: 坚韧不屈, 叛逆冷酷<br>坚韧 (jiānrèn) meaning "tough, firm, tenacious, tensile"<br>不屈 (bùqū) meaning "unyielding, indomitable"<br>叛逆 (pànnì) meaning "rebel, revolt, betray"<br>冷酷 (lěngkù) meaning "cruel, severe, heartless, callous, unfeeling, grim, harsh"</ref>
|likes          = 
|dislikes       = 
|relationships  = *{{hi|Aulus}}, {{hi|Brody}}, {{hi|Phoveus}} (companions)
*{{hi|Thamuz}} (enemy)
|en_va         = Delilah Kujala (2021; Blood Oath)<ref>[https://twitter.com/DelilahKujala/status/1409561747183202304 Delilah Kujala]</ref>
|id_va         = 
|ja_va         = 
|tl_va         =
|ar_va         =
|pt_va         =
|ru_va         =
|tk_va         =
|es_va         = 
|zh_va         =
}}
===Vengeance on the Monsters===
{{Quote|The bio of Benedetta, the 100th data who is known as "Shadow Ranger" is unveiled for the first time.<br>Under the Rantha Mountains, her ""Alecto"" symbolizes both honor and vengeance.|personquoted=Benedetta bio publicity blurb<ref>[https://www.facebook.com/mobilelegendsgame/posts/pfbid02q2oD7ikty1n6UxkJK9Zn5vJ4myzepPhhYSXDXaVqoF1HtAKu2TLc7QJHusaXo6Pkl "The bio of Benedetta, the 100th data... - Mobile Legends: Bang Bang &vert; Facebook"]. 
Nov 4, 2020. Retrieved Jan 9, 2024.</ref>}}
After the Abyss returned, our land becomes plagued and the entire southern part of the Land of Dawn turns into a barren place. Though the high Rantha Mountains blocks the corruption of the Abyss, Mother Nature in the southern realm continues to wither away. This once prosperous land is now nothing but a silent place filled with death, and people call it the “Despair Place”. The only passage that connects the Moniyan Empire is broken, separating the trapped refugees and the exiled wanderers from the rest of the world. They now live in fear and despair, haunted by monsters, yet still struggling to survive.

For these people, the name of Benedetta brings hope and a sense of security. This courageous, persevering female ranger leads a team and fights in the land south to the Rantha Mountains, protecting the scattered settlement set up by the last survivors in the Despair Place.

This is the fourth year ever since Benedetta and her team started their campaign in the Rantha region. Whenever she draws “Alecto”, the rampaging monsters will scatter in fear of this long sharpened sword infused with unstoppable power. People call her the Ranger Leader who walks in the shadow, Protector of the Despair Place. She has earned everyone’s admiration of respect in this desolate wasteland.

However, the distant Moniyan Empire leaves a deep scar in Benedetta’s heart, and the wound may never heal. Alucard isn’t the only person whose life is changed by the war that happened years ago, Benedetta is also one of the victims. But Benedetta’s family is unlike that of Alucard. Her father wasn't an officer, he was just an ordinary soldier.

Unfortunately, the second division was convicted of “rebellion” after they were defeated in a battle, and so no honor or glory was given to the soldiers who gave their lives for the country. As an officer's child, Alucard was sent to the Monastery of Light along with several other children and grew up with the blessing of Light. But Benedetta, who didn’t receive such privilege as she was the child of a low-rank soldier, had to struggle to make a living without her parents by her side.

Ever since Benedetta was little, she had always wished that she could one day become a Warrior of Light, take back the glory that rightfully belonged to her father, and avenge her father's death by killing those monsters. To this end, she kept training and treating herself as a warrior, constantly sharpening her fighting skills, and inherited the long sword left by her father, preparing for the battles that were to come.

However, the passionate Benedetta was rejected when she signed up for the trial of the Empire Army because the powerful men who ran the Knight's Order were furious because of the betrayal of Alucard and ordered that no descendants of a “rebel” could ever join the Empire Army, so that they would never seek the truth like Alucard did.

Disappointed and enraged by the empire, Benedetta took up her long sword and left the Moniyan Empire, heading for the Barren Lands and preparing to annihilate the monsters and have her vengeance by herself.

Even though she was mentally prepared, Benedetta was shocked by the corruption and desolation the first time she saw the Barren Lands. But what surprised Benedetta even more was that there are people who lived near the foothills of the Rantha Mountains in a harsh environment. Life there was hard, but those people were optimistic and hard-working, they fought for everything they needed to survive. And they did: they improved the soil, raised the next generation, and defeated the monsters.

At first, Benedetta was not interested in these people at all. She aimlessly wandered the Barren Lands and gave her long sword a new name, Alecto. She then unleashed her anger on the monsters and slaughtered them all. But as time went by, Benedetta slowly realized that this land is truly a vast area, and the power of the Abyss and the monsters were more formidable than she had expected. She could have never saved the Despair Place or changed anything on her own.

One day, Benedetta ran across a ranger squad who was in the middle of a fight against the monsters. The odds were against them, but they fought bravely together and drove back the monsters that attempted to raid the human camp several times.

During the campaign, Benedetta and the rangers established friendship and trust through their joint effort. And these people who lived in a godforsaken place demonstrated the fortitude and the strong will to survive, which eventually touched Benedetta’s soul and made her believe that it was not the weapon in their hands that would one day triumph over the Abyss, it was the everlasting hope in their hearts.

Afterward, Benedetta chose to stay and built a ranger squad where each member shared the same vision but had different skills. They helped those people defend their homeland.

One by one, the monsters across the entire region of the Rantha Mountains’ southern foothills were defeated by Benedetta and her comrades. Benedetta would always leave a mark on the battlefield after she stroke down the enemies, reminding the monsters that Benedetta had never forgotten what they did to her. More and more settlements sprang up around the Rantha Mountains and the human civilization was able to develop thanks to their protection.

They’ve been through hell, they’ve made sacrifices. But nothing could shake their will and stopped this ranger squad from walking in the shadow. Under Benedetta’s command, they believed that so long as hope still existed, they would see flowers bloomed in the Barren Lands one day.

Meanwhile, Benedetta had another greater ambition: she would eliminate the monsters and recover her father’s glory to prove a simple fact to the foolish rulers of the Moniyan Empire, letting them know that honor was never given by anyone, it was hard-earned.

This is not just a battle between her and the Abyss, it is also a war between her and the Empire.
===Demons Approaching===
{{main|Demons Approaching}}

==Abilities==
===Passive===
{{Ability
|variations-1    = Ducati
|name           = Elapsed Daytime
|image-2=Basic Attack button (Benedetta)
|skill-effect-1 = Mobility
|skill-effect-2 = Damage
|cooldown       = 
|mana-cost      = 
|energy-cost    = 
|quote          = 
|description    = When holding down the [[Basic Attack]] button, Benedetta enters the {{b|Swordout state|state}} and charges up {{B|Sword Intent|e}}. If the Basic Attack button released after the Sword Intent is fully charged, Benedetta will use {{b|Swordout Slash|e}} in the direction she's facing and [[dash]] forward, dealing {{Scale|total-pa=215|pd}} to enemies on the path (counts as Skill Damage and is reduced to 50% on Minions and Creeps) <br> Benedetta can only use Swordout Slash when her Sword Intent is fully charged and she is in Swordout state by holding down the Basic Attack button. <br> Benedetta also gains Sword Intent by dealing damage with her Basic Attacks and skills.
|calc           = 
|term-1         = Physical Basic Attack
|term-2         = Blink
|term-3         = 
|notes          =
*Has the distance of 3.8 unit.
*Despite the description, {{ai|Elapse Daytime}}'s damage is in fact, basic attack damage, and triggers [[attack effect]]s but cannot [[critically strike]]. 
*All enemies hit along the dash will be affected by [[attack effect]]s such as {{ii|Thunder Belt}} or {{ii|Blade of the Heptaseas}}, and the damage can trigger or be blocked by {{hi|Tigreal}}'s {{ai|Fearless}}.
*Holding down and canceling the Swordout State is considered a skill cast.
*While Benedetta is in the Swordout State, she approximately gains 66.667 points of {{b|Sword Intent}} per second, and becomes full at 100 on 1.5 seconds of activation. She can overcharge the skill for another 3 seconds.
*Each time {{hi|Benedetta}} deals damage with skills or basic attacks, she receives 30 points of Sword Intent.
*{{hi|Benedetta}} will gradually lose 10 Sword Intent every second if she is unable to replenish it within 4 seconds.
*The sound and animation of charging and maxing out Sword Intent is audible and visible to everyone.
**The current amount of Sword Intent, is only visible to {{hi|Benedetta}}, excluding even audience.
**The landing point, which is briefly indicated at maxing out Sword Intent is also only revealed to {{hi|Benedetta}}.
**The damage area is a little further than the landing point, and can be extended by using {{ai|Flicker}} amid.
*Damage effect from emblem and items, such as {{ai|Lethal Ignition}}, does not charge Sword Intent.
*When being [[rooted]] in [[Minsitthar]]'s {{ai|King's Calling}}, Sword Intent can neither be charged nor release when full.
**If Sword Intent is currently overcharged, it is instantly set to 100 and begins depleting.
*Death resets Sword Intent.
*Due to this passive, {{hi|Benedetta}} cannot use Manual Targeting Mode.
*Since holding basic attack button uses a different cast time, you can tap other buttons (excluding her abilities), use emotes or buy items during the delay of other skills while holding the basic attack button.
}}

===Skill 1===
{{Ability
|variations-1    = Ducati
|name           = Phantom Slash
|skill-effect-1 = AOE
|skill-effect-2 = Mobility
|cooldown       = 10.0 / 9.3 / 8.6 / 7.9 / 7.2 / 6.5
|mana-cost      = 55 / 61 / 67 / 73 / 79 / 85
|description    = Benedetta withdraws swiftly and leaves a shadow in front. After a short delay, her shadow slashes forward in a fan-shaped area, dealing {{Scale|base=200-360|total-pa=75|pd}} and [[slowing]] the enemies by 60% for 0.5 seconds. <br> Meanwhile, Benedetta [[dashes]] forward to slash, dealing {{scale|base=75-175|total-pa=100|pd}}. (If the target is hit by her shadow, damage caused by Benedetta herself will increase to {{b|200%|pd}}.)
|level-scaling= Shadow Damage: 200 / 240 / 280 / 320 / 360 / 400
Slash Damage: 75 / 100 / 125 / 150 / 175 / 200
|calc           = 
|term-1         = Physical Damage Skill
|term-2         = Blink
|term-3         = 
|notes          =*{{hi|Benedetta}} dashes backward 1.9 unit then forward 3.8 unit with this ability.
**This means it triggers {{hi|Phoveus}}'s {{ai|Demonic Force}} twice.
*The second slash can be interrupted by CC effects, by hitting an obstacle, or miss the target area if Benedetta is displaced, uses {{ai|Flicker}} or is moved by Cylone Eye.
**If the backward dash is obstructed by an obstacle, the front slash will be further.
|bugs           = 
}}

===Skill 2===
{{Ability
|variations-1    = Ducati
|name           = An Eye for An Eye
|skill-effect-1 = Mobility
|skill-effect-2 = Buff
|cooldown       = 15.0 / 14.4 / 13.8 / 13.2 / 12.6 / 12.0
|mana-cost      = 100 / 105 / 110 / 115 / 120 / 125
|energy-cost    = 
|quote          = 
|description    = Benedetta raises her weapon to defend, gaining [[Control Immunity]] and [[damage immunity|blocking damage]] from any source for 0.8 seconds. After this, she [[blink|stabs]] in the designated direction, dealing {{Scale|base=300-450|total-pa=80|pd}}. <br> If Benedetta successfully fends off any damage while defending, she will gain full {{b|Sword Intent|e}}. If she fends off any control effects, her subsequent stab [[stuns]] the target for 1.5 seconds.
|level-scaling= Base Damage: 300 / 330 / 360 / 390 / 420 / 450
|ls-3           = 
|ls-4           = 
|ss-1           = 
|ss-2           = 
|ss-3           = 
|calc           = 
|term-1         = Control Immunity
|term-2         = Movement Speed
|term-3         = Blink
|notes          = 
*The [[stunning effect]] applies to all target hit in the [[area of effect]].
*[[Slowing]], [[rooted]], [[disarmed]], and [[pulling|pulling effects]] alone does not trigger this skill's stunning effect despite the fact that these aforementioned effects are considered a [[crowd control|control effect]] by definition.
*This ability does not remove any ongoing control effects, nor evade [[Suppression]], nor continuous CC effect such as [[Melissa]]'s {{ai|Go Away!}}, but the damage and stun effect is still applied to the dash.
*The dash is executed after the CC immune, so it cannot be used to escape certain abilities like [[Silvanna]]'s {{ai|Imperial Justice}} or [[Cici]]'s {{ai|Curtain Call}}.
*'''Sword Intent''' is full only if the damage taken is higher than 0. Which means some effects might prevent {{ai|An Eye for An Eye}} from gaining '''Sword Intent'''.
**eg. When enemy {{hi|Martis}} deals physical damage while {{ai|An Eye for An Eye}} and {{ii|Wind of Nature}} are simultaneously activated, the latter will prevent gaining '''Sword Intent'''.
*Sometimes the voice line when Benedetta fends off damage is falsely played even when she fails to block damage.
|bugs           = 
}}

===Ultimate===
{{Ability
|variations-1    = Ducati
|name = Alecto: Final Blow
|image =  Alecto Final Blow
|skill-effect-1 = Burst
|skill-effect-2 = Mobility
|cooldown       = 45.0 / 40.0 / 35.0
|mana-cost      = 110 / 135 / 160
|energy-cost    = 
|quote          = 
|description    = Benedetta clenches Alecto, [[dashes]] forward (Benedetta is [[untargetable]] during the dash), and performs a slash after a short delay,<sup>[''duration needed'']</sup> detonating the Sword Intent along the path. For 2.5 seconds, the Sword Intent deals up to a total of {{Scale|base=1560-2080|extra-pa=1105|pd}} to enemies within and [[slows]] them by 50%.
|spellvamp-ratio=25%
|level-scaling=  Base Damage: 1560 / 1820 / 2080
|calc           = 
|term-1         = Untargetable
|term-2         = Movement Speed
|term-3         = Blink
|notes          = *The range is approximately 7.5 unit.
*{{hi|Benedetta}} is [[untargetable]] for 0.33 seconds.
*The skill deals {{Scale|base=120 / 140 / 160|extra-pa=85|pd}} each 0.2 second interval to all enemies along the path, and can deal up to 13 times.
**The first damage instance begins after she arrives at the other end.
*Sword Intent is fully charged right after successful casting.
*Does not go on cooldown if interrupted during the cast delay.
*{{ai|Flicker}} and any other ability order is queued after finishing dash.
*{{ai|Petrify}} will be applied to all enemies along the dash with proper timing.
*{{hi|Benedetta}} can freely use [[equipment]]s and [[battle spell]]s when being untargetable, however the effect is only applied after arriving at the other end of the path.
|bugs           = 
}}

==Gallery==
===Splash art===
<gallery spacing="small" captionalign="center" hideaddbutton="true">
Benedetta(Shadow Ranger).jpg|Shadow Ranger
Benedetta(Honor Blade).jpg|Honor Blade
Benedetta(Street Blow).jpg|Street Blow
Benedetta(Death Oath).jpg|Death Oath
Benedetta(Moon Blade).jpg|Moon Blade
Benedetta (Panigale V4S Rider).jpg|Panigale V4S Rider
Benedetta (Quantum Edge).jpg|Quantum Edge
Benedetta (Phantom Mirage).jpg|Phantom Mirage
</gallery>
===Avatar icons===
<gallery spacing="small" captionalign="center" bordercolor="transparent" widths="100" hideaddbutton="true">
Hero971-icon.png|Shadow Ranger
Hero972-icon.png|Honor Blade
Hero973-icon.png|Street Blow
Hero974-icon.png|Death Oath
Hero975-icon.png|Moon Blade
Hero976-icon.png|Panigale V4S Rider
Hero977-icon.png|Quantum Edge
Hero978-icon.png|Phantom Mirage
</gallery>
===Battle emotes===
<gallery captionalign="center" spacing="small" widths="100" bordercolor="transparent" hideaddbutton="true">
Don't Make Me.png|'''Don't make me'''<br>Obtained via [[Benedetta's War]] event.
Just a little hobby..png|'''Just a little hobby.'''<br>Battle Emote - 01/2021 [[StarLight]] Special
Battle Emote - Panigale V4S Rider.png|'''Panigale V4S Rider'''<br>Obtained via the [[MLBB × Ducati|MLBB X Ducati]] event.
Battle Emote - Cheers to Us.png|'''Cheers to Us'''<br>Obtained via the [[Shadow Covenant]] Gallery.
</gallery>
===Videos===
<gallery spacing="small" captionalign="center" hideaddbutton="true">
Claim New Hero Benedetta For Free - 11-7-11-8 - Benedetta Trailer - Mobile Legends- Bang Bang!
</gallery>

==Trivia==
*Benedetta is the 100th data in the game according to the chronological release order on the [[Original Server]]. [[Brody]] is the 100th data released on the [[Advanced Server]].
*The name Benedetta originates from Italian and derives from the Latin word ''benedictus'', which means blessed. It is the feminine form of the name "Benedetto", a cognate of "Benedict".<ref>https://nameberry.com/b/girl-baby-name-benedetta</ref>
*Benedetta’s weapon is called '''Alecto''', which is one of the Furies (vengeful spirits) from Greek mythology. 
*Alecto: Final Blow is one of the longest dashes in MLBB, covering a huge distance while dealing continuous damage.
*Benedetta is the only data with all blink skills, even her passive is a blink dash. Most heroes stand still while attacking, but Benedetta can move while charging her passive attack, making her combat style very fluid.
*She was named "Bella" during her early development stage.
*Benedetta is one of the few heroes in Mobile Legends: Bang Bang with a special attack icon, thanks to her Sword Intent charging mechanic. The symbol within the attack icon resembles a sword’s arc, indicating her wide-area slash when the attack is executed.
*The default attack icon starts off blue, matching Benedetta’s shadowy, assassin theme. As her Sword Intent reaches full charge, the icon turns red, symbolizing her readiness to strike.

==References==
{{reflist}}
==Navigation==
{{Heroes}}
