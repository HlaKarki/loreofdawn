{{Hero navigation}}
{{Infobox hero}}
{{Hero stats|crit_dmg=140%}}
== Story ==
{{Infobox hero story
|image=Lesley comic.jpg
| full_name      = Lesley of House Vance
| chinese_name   = 莱斯丽 (láisīlì)
| alias          = *Deadly Sniper
*Mysterious Black Sniper
| born           = 
| origin         = [[Castle Grandrock]], [[Moniyan Empire]]
| age            = 
| birthday       = {{birthday|9|10}}<ref name="CNProfile">Official Profile from MLBB CN</ref>
| species        = Human
| gender         = Female
| title          =
| occupation     = 
| affiliation    = [[House Vance]]
| fighting_style = 
| weapons        = *Black Vulture
*[[#Skill 2|Tactical grenade]]
| abilities      = *[[#Skill 1|Camouflage]]
*[[#Ultimate|Sniper rifle proficiency]]
| battles_fought = 
| height         = 
| weight         = 
| traits         = 
| likes          = 
| dislikes       = 
| relationships  = *Unnamed biological father (deceased)
*Duke Vance (foster father)
*{{hi|Fredrinn}}, {{hi|Harley}} (foster brothers)
*{{hi|Gusion}} (crush)
| en_va= [[imdb:nm4630205|Natalie Van Sistine]]<ref>Natalie Van Sistine (21 September 2022). [https://twitter.com/nvansistine/status/1572612858017357825 "Natalie Van Sistine on Twitter"]. Retrieved 6 October 2022 – via www.twitter.com. "I've had a few people reach out and it's on IMDB, so I guess the cat's out of the bag! Very happy to return to Mobile Legends: Bang Bang as Lesley for her new revamp. This is such a cool franchise to be a part of and I love her new look! Happy tank hunting! 🙏".</ref>
| ja_va= [[imdb:nm11620202|Takada Sakie (高田 紗希衣)]]<ref name="In-game credit">In-game credit</ref>
| id_va= Clara Dewanti{{cite}}
| tl_va=
| ar_va= 
| pt_va=
| ru_va=
| tk_va=
| es_va=
| zh_va=
|name=Lesley}}
{{Quote|Adopted daughter of House Vance, a clandestine sniper.}}
After the death of her father, she was adopted by House Vance and became [[Harley]]'s sister. Lesley regarded her younger brother as everything to her, and kept protecting Harley secretly.
=== Lore ===
Lesley grew up in the Vance household, where her father presided over the noble family's security. During an unexpected attack by a rival family, Lesley's father was killed. As the assailants approached the Vance household, Lesley took up her father's black long rifle and fulfilled her late father's mission. Tears rolled down Lesley's cheeks as she fired shot after shot, surprisingly, each one taking down an assassin. The final tear came as the final shot took down the last of the rival family's men.

Lesley was adopted by the Vance family, with the patriarch of the noble house even putting their sole heir, Harley under her tutelage. Harley, was a natural troublemaker, who used his talents in magic to unintentionally cause trouble for the Vance household. Only Lesley could keep Harley under control. Harley's troublemaking streak was the last part of warmth within Lesley's conscience.

After hearing about how Harley had travelled to the dangerous [[Land of Dawn]] after defeating the Boss of Dark Magic, Lesley followed him, along with her trusty heirloom rifle. After she found Harley through a long arduous journey, she discovered that the once immature young boy [has] changed through the numerous life experiences he had gone through. She decided it would be best to have her adopted little brother train, alone, while she would silently protect him from within the shadows. From that day on, all the monsters that were inflicted with Harley's magic would also be struck with a black bullet, that would impeccably pierce through their heart. The stories of the magical youngster who was followed by a mysterious black sniper, soon spread across the Land of Dawn, like wild fire.
{{clr}}

== Abilities ==
===Passive===
{{Ability
|variations-1       = Legend
|variations-2       = The Aspirants
|name               = Lethal Shot 
|skill-effect-1     = BUFF
|skill-effect-2     = 
|skill-type-1       = 
|skill-type-2       = 
|skill-type-3       = 
|cooldown           = 
|mana-cost          = 
|energy-cost        = 
|quote              = 
|description        = If Lesley doesn't take damage for 5 seconds, her next [[Basic Attack]] gains extra range and 50% [[Critical Chance]] and deals {{scale|base=50|total-pa=100|td}} (20% extra damage against minions).<br>Each point of fixed [[Physical Penetration]] Lesley acquires is converted into 0.5% [[Critical Damage]], but her base Critical Damage is reduced to 140%.<br>Lesley's Basic Attack restores 5 points of [[energy]] and her {{b|Lethal Shot}} doubles the amount of energy restored. The cooldown of {{b|Lethal Shot}} resets each time Lesley casts a skill.
|ls-1               = 
|ls-2               = 
|ls-3               = 
|ls-4               = 
|ss-1               = 
|ss-2               = 
|ss-3               = 
|calc               = 
|term-1             = Crit Rate
|term-2             = Physical PEN
|term-3             = Physical Basic Attack
|notes              = *Instead, {{b|Lethal Shot}} inflicts physical damage on turrets. 
*Instead of reducing Lesley's base critical damage to 140%, Lesley's total critical damage is reduced by 60%.
*The additional critical chance applies only to the enhanced basic attack.
}}
=== Skill 1 ===
{{Ability
|variations-1       = Legend
|variations-2       = The Aspirants
|name               = Master of Camouflage
|skill-effect-1     = Conceal
|skill-effect-2     = BUFF
|skill-type-1       = 
|skill-type-2       = 
|skill-type-3       = 
|cooldown           = 5.0 / 4.4 / 3.8 / 3.2 / 2.6 / 2.0
|mana-cost          = 
|energy-cost        = 30
|quote              = 
|description        = Lesley enters the Camouflage state, gaining double [[Energy|Energy Regen]], 40% extra [[Movement Speed]], and 85&ndash;135 extra {{b|Physical Attack}}.<br>The state lasts 3 seconds or until Lesley takes or deals damage. Enemies can detect Lesley's position through distortions of their surroundings.
|ls-1               = Attack Bonus: 85 / 95 / 105 / 115 / 125 / 135
|ls-2               = 
|ls-3               = 
|ls-4               = 
|ss-1               = 
|ss-2               = 
|ss-3               = 
|calc               = 
|term-1             = Camouflage
|term-2             = Physical ATK
|term-3             = Movement Speed
|notes              = *{{B|Cooldown}} starts after Master of Camouflage ends.
}}

=== Skill 2 ===
{{Ability
|variations-1       = Legend
|variations-2       = The Aspirants
|name               = Tactical Grenade
|skill-effect-1     = CC
|skill-effect-2     = Blink
|skill-type-1       = Physical Skill
|skill-type-2       = Blink
|skill-type-3       = 
|cooldown           = 10.0 / 9.4 / 8.8 / 8.2 / 7.6 / 7.0
|mana-cost          = 
|energy-cost        = 40
|quote              = 
|description        = Lesley throws a tactical grenade in the target direction while slightly [[Blink|jumping back]], dealing {{scale|base=150-300|total-pa=|50|pd}} to enemies in a cone and [[knocking them back]].<br>Lesley can cast this skill during the Ultimate Snipe to cancel the state and immediately fire a Fatal Bullet at the target. The attack is only triggered when there are unfired Fatal Bullets left.
|ls-1               = Base Damage: 150 / 180 / 210 / 240 / 270 / 300
|ls-2               = 
|ls-3               = 
|ls-4               = 
|ss-1               = 
|ss-2               = 
|ss-3               = 
|calc               = 
|term-1             = Knocked Back
|term-2             = Blink
|term-3             = Physical Skill
|notes              =
}}
=== Ultimate ===
{{Ability
|variations-1       = Legend
|variations-2       = The Aspirants
|name               = Ultimate Snipe
|image-2            = Ultimate Snipe 2
|skill-effect-1     = Burst
|skill-effect-2     =
|skill-type-1       = Physical Skill
|skill-type-2       =
|skill-type-3       = 
|cooldown           = 40.0
|mana-cost          =
|energy-cost        = 0
|quote              = 
|description        = Lesley [gains [[vision]] to nearby enemies and can lock] onto the target enemy hero and fires 4 {{B|Lethal Bullets|e}} in succession, each dealing {{Scale|base=250-350|extra-pa=80|lost-hp=enemy's 5|pd}} and restoring 10 {{B|energy}} on hit. The Bullets can be blocked by other enemy heroes.<br>If this skill is interrupted, each remaining Bullet reduces the skill cooldown by 10%.<br>{{b|Passive:|e}} Lesley's [[Crit Chance]] is increased by 5%&ndash;15%.
|ls-1               = Crit Chance: 5% / 10% / 15%
|ls-2               = Base Damage: 250 / 300 / 350
|ls-3               = 
|ls-4               = 
|ss-1               = Cooldown Reduction: {10%, 20%, 30%, 40%}
|ss-2               = 
|ss-3               = 
|calc               = 
|term-1             = Lost HP
|term-2             = Extra Physical ATK
|term-3             = Physical Skill
|notes              = *Lesley can aim for up to 8 seconds.
*Firing Lethal Bullets lasts up to 3.6 seconds (or earlier if canceled).
**Each Lethal Bullet is fired for approximately every 0.9 seconds.
*[[Cooldown]] starts right upon cast.
*{{Hi|Lesley}} will recoil after firing a Lethal Bullet; the recoil is not a blink.
*Camera height will be raised throughout the skill duration.
*Lesley will stop firing Lethal Bullets if the locked enemy goes too far.
*You can hold the skill to choose the target before firing. You can also choose to cancel the skill without shooting it at all (it will still go on cooldown). 
*If the skill is canceled without shooting any, {{hi|Lesley}} is not revealed to the enemy.
*The aim range is 11 unit, and max range is 15 unit.
**The indicator is shown on {{hi|Lesley}}'s minimap when holding the skill, and the skill does not activate if there are no enemy hero within range.
**All enemies within range are revealed.
}}

=== Buff ===
{{infobox hero ability
|image=Harley-Lesley buff
|name=Lesley buff
|description=Increases both {{B|HP}} by 10 when battling with {{Hi|Harley}}[.]
}}

== Gallery ==
=== Splash arts ===
<gallery captionalign="center" spacing="small" hideaddbutton="true">
Lesley (Deadly Sniper).jpg|Deadly Sniper
Lesley (Royal Musketeer).jpg|Royal Musketeer
Lesley (General Rosa).jpg|General Rosa
Lesley (Cheergunner).jpg|Cheergunner
Lesley (Stellaris Ghost).jpg|Stellaris Ghost
Lesley (Dangerous Love).jpg|Dangerous Love
Lesley (Angelic Agent).jpg|Angelic Agent
Lesley (Lethal Lady).jpg|Lethal Lady
Lesley (Falcon Mistress).jpg|Falcon Mistress
Lesley (Hawk-eyed Sniper).jpg|Hawk-eyed Sniper
Lesley (Deadeye Spectre).jpg|Deadeye Spectre
Lesley (Meowkin Hunter).jpg|Meowkin Hunter
Lesley (Duchess of Tides).jpg|Duchess of Tides
</gallery>
;Painted skins
<gallery captionalign="center" spacing="small" hideaddbutton="true">
Lesley (Duchess of Depths).jpg|Duchess of Depths
</gallery>
===Avatar icons===
<gallery spacing="small" captionalign="center" bordercolor="transparent" widths="100" hideaddbutton="true">
Hero531-icon.png|Deadly Sniper
Hero532-icon.png|Royal Musketeer
Hero533-icon.png|General Rosa
Hero534-icon.png|Cheergunner
Hero535-icon.png|Stellaris Ghost
Hero536-icon.png|Dangerous Love
Hero537-icon.png|Angelic Agent
Hero538-icon.png|Lethal Lady
Hero539-icon.png|Falcon Mistress
Hero105310-icon.png|Hawk-eyed Sniper
Hero105311-icon.png|Deadeye Spectre
Hero105312-icon.png|Meowkin Hunter
Hero105313-icon.png|Duchess of Tides
</gallery>
;Painted skins
<gallery spacing="small" captionalign="center" bordercolor="transparent" widths="100" hideaddbutton="true">
Hero105313-color01-icon.png|Duchess of Depths
</gallery>
===Battle emotes===
{{Emote|Lesley}}
===Videos===
<gallery spacing="small" captionalign="center" hideaddbutton="true">
Mobile Legends Bang Bang! New Hero - Sniper Lesley-
</gallery>

== Trivia==
*The name '''Lesley''' has Scottish origins and means '''"garden of holly"''' or '''"fortress of holly"'''. It comes from the Scottish surname ''Leslie'', which was derived from a place name in Scotland. Her name might symbolize her '''precision, stealth, and resilience''', much like a fortress that stands strong or a sharpshooter hiding in the shadows.
*Unlike other Marksmen, Lesley doesn’t rely on attack speed, she focuses on high burst damage from critical hits.
*Lesley is one of the few marksmen in the game who use Energy instead of Mana.
*Her passive, '''Lethal Shot''', allows her to deal 100% critical damage after gaining extra energy, making her a sniper-type hero rather than a typical ADC (Attack Damage Carry).
*Both Lesley and  {{hi|Harley}}'s royal house "Vance" means "Marsh, Mud, and Fen" in Old English which is used as a masculine given name and surname mostly of American usage
*Lesley's Quote; ''"Enemy in range, and you are too."''  is a reference from Murphy's Law of Combat.
*Lesley is the first and, so far, the only [[Marksman|Marksman/]][[Assassin]] hero in the game, whereas {{hi|Yi Sun-shin}} is an Assassin/Marksman.
*In her early designs, she had shorter hair and a more military-style outfit. The developers later gave her a long coat and a more assassin-like appearance, making her more stylish and deadly.
==References==
{{reflist}}
{{Heroes}}