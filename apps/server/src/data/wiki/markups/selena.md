{{Hero navigation}}
{{Infobox data}}
{{Hero stats}}
== Story ==
{{Infobox data story
| full name      = 
| alias          = Abyssal Witch
| born           = [[Shadow Swamp]], [[Azrya Woodlands]]
| origin         = 
| species        = Dark Elf
| gender         = Female
| title          = 
| occupation     = 
| affiliation    = 
| fighting_style = 
| weapons        = Abyssal Devil 
| abilities      = 
| battles_fought = 
| height         =
| weight         = 
| traits         = 
| likes          = 
| dislikes       = 
| relationships  = *{{hi|Karina}} (older sister)
*{{hi|Aldous}}, {{hi|Alice}}, {{hi|Obsidia}} (enemies)
| en_va=
| ja_va= Nokubi Naoko (野首 南帆子)<ref name="In-game credit">In-game credit</ref>
| id_va= Jane Leisilla Zahara<ref>[https://www.tiktok.com/@ilazahara/video/7077449778019192090?is_from_webapp=1&sender_device=pc&web_id=7070120765835806210 Jane Leisilla Zahara (@ilazahara) TikTok]</ref>
| tl_va=
| ar_va=
| pt_va=
| ru_va=
| tk_va=
| es_va=
| zh_va=
}}
{{Quote|Dark Elf girl who fell into the Abyss}}
The Dark Elf Selena had been dependent on her sister Karina since childhood. Selena maintained her innocent nature under the protection of her sister, until she discovered the dirty things her sister Karina had done for her. She thought that she had become a burden for her sister. In order to gain more power, she dedicated herself to the Abyss, but did not want to be completely controlled by the Abyss and ultimately became a horrifying demon.
=== Lore ===
South of the [[Moniyan Empire]] lies a bottomless abyss, engraved into this barren landscape as if an ancient demon used the sharpest of blades to carve a disfiguring scar into the [[Land of Dawn]]. People call this the [[Shadow Abyss]].

Sealed away at the bottom of this legendary abyss, lies the most terrible demon in the Land of Dawn - the Abyss Dominator. But as the seal slowly fades, the will of the abyss gradually grows stronger.

The demons at the bottom of the abyss tirelessly devise plots to strengthen the power of darkness, persuading misguided believers to provide them with sacrifices for their evil cause. They use these sacrifices to feed the 'Abyss Devil', a monster formed of the purest dark energy that waits in the darkness to devour its prey.

Selena, a young Dark Elf, was sacrificed to the Shadow Abyss by fanatics in exchange for dark energy, and as Elves are the Abyss Devil's favorite sacrifices, Selena was offered up immediately by the demons. The Abyss Devil maniacally drilled into Selena, severe pain and desperation spreading through every corner of the young Elf girl's body. Just as she was about to be consumed by dark magic, a miracle occurred - a warm blue light enveloped the girl, and the dark abyssal magic and this mystical power began waging a savage battle inside her fragile, young body.

Selena awoke find herself lying in the depth of the abyss, her skin now a mysterious blue tone and her lavender eyes filled with a scarlet glow, with the Abyss Devil lying quietly at her side.

The Demons of the Abyss were busy wondering what had just happened. Little did they know, the will of the Abyss had arrived. This girl is loved deeply, and this love has protected her from being consumed by darkness. The Abyss and the Moon Goddess - these two opposing forces have reached a balance within her body. An ancient, heavy voice echoed through the minds of the demons, "Leave this girls to me. She will become the strongest warrior of the Shadow Abyss!" The demons obeyed the abyss' command and left her there. When Selena reappeared before the demonic inhabitants of the abyss, with the Abyss Devil at her side, she had completely forgotten her past.

She found that she could skillfully manipulate the abyss to release dark magic, a demon capable of tearing her enemies to shreds. The will of the Shadow Abyss appointed her as its voice, making it her duty to rule over the demonic denizens. Even the proud Queen of Blood Magic began to feel unnerved upon seeing this young Dark Elf Girl, not only because the abyss had chosen her, but also because [[Alice]] could sense the terrible power lying within this seemingly cute and mischievous youngster.

== Abilities ==
=== Passive ===
{{Ability
|variations-1   = Zenith
|name           = Symbiosis
|skill-effect-1 = BUFF
|description    = Selena can switch freely between her two forms. {{b|Elven Form|e}}: Selena applies {{b|"Abyssal Mark"|e}} with her skills. The mark can stack up to 2 times. When {{ai|Abyssal Trap|"Abyssal Trap"}} hits an enemy, it immediately adds an "Abyssal Mark". {{b|Abyssal Form|e}}: All damage will consume 1 stack of "Abyssal Mark" to deal {{Scale|base=225|level=15|total-mp=40}} extra {{b|Magic Damage}}.
|ls-1=Base Damage: 240 / 255 / 270 / 285 / 300 / 315 / 330 / 345 / 360 / 375 / 390 / 405 / 420 / 435 / 450
|term-1          = Magic Skill
}}

===Skill 1===
<tabber>
|-|Elven Form=
{{Ability
|variations-1   = Zenith
|name           = Abyssal Trap
|cooldown       = 8.0 / 7.6 / 7.2 / 6.8 / 6.4 / 6.0
|mana-cost       = 60 / 70 / 80 / 90 / 100 / 110
|skill-effect-1 = Summon
|skill-effect-2 = Slowed
|quote          = The iron will of the Abyss befalls.
|description    = Selena summons an Abyssal Devil to lurk in a designated location. The Abyssal Devil will attach itself to any enemy that approaches the location and [[slow]] them by 70%. After 1 second, the Abyssal Devil deals {{Scale|base=350-550|total-mp=150|md}} to the enemy and other nearby enemies and slows them by 50% for 1 second. The Abyssal Devil lasts up to 60 seconds. Up to 3 Abyssal Devils can exist at the same time. Each extra Abyssal Devil increases the final damage by 50%.
|ls-1           = Base Damage: 350 / 390 / 430 / 470 / 510 / 550
|term-1         = Magic Skill
|term-2         = Movement Speed
|notes          = *The Abyssal Devils grant vision to the surrounding area.
}}
|-|Abyssal Form=
{{Ability
|variations-1   = Zenith
|name           = Soul Eater
|cooldown       = 6.0 / 5.5 / 5.0 / 4.5 / 4.0 / 3.5
|mana-cost      = 60 / 65 / 70 / 75 / 80 / 85
|skill-effect-1 = Burst
|skill-effect-2 = Shield
|quote          = The iron will of the Abyss befalls.
|description    = Selena strengthen her claws with the power of the Abyss, causing the next [[basic attack]] to deal extra {{Scale|base=200-350|total-mp=130|md}}, while granting a Shield that absorbs {{Scale|base=300-400|total-mp=150|shield}}
|ls-1           = Base Damage: 200 / 230 / 260 / 290 / 320 / 350
|ls-2           = Basic Shield: 300 / 320 / 340 / 360 / 380 / 400
|term-1         = Magic Skill
|term-2         = Shield
|notes=
}}</tabber>
=== Skill 2 ===
<tabber>
|-|Elven Form=
{{Ability
|variations-1   = Zenith
|name           = Abyssal Arrow
|cooldown       = 12.0
|mana-cost       = 70 / 80 / 90 / 100 / 110 / 120
|skill-effect-1 = CC
|skill-effect-2 = Speed Up
|description    = Selena fires magic arrows in a designated direction, dealing {{Scale|base=250-400|total-mp=20}} to {{Scale|base=500-800|total-40|md}} to the first enemy data hit and [[stunning]] it for 0.5 seconds to 3 seconds. Stun duration and damage increase with the distance that the arrows travel. The arrow absorbs {{ai|Abyssal Traps|"Abyssal Traps"}} as it travels and hence will apply effects of "Abyssal Traps" to the enemy target upon hit. If an enemy is stunned for 1 second or longer, Selena's [[Movement Speed]] will be increased by 40% for 2 seconds.
|ls-1           = Base Damage: 250 / 280 / 310 / 340 / 370 / 400
|ls-2           = [Base] Max Damage: 500 / 560 / 620 / 680 / 740 / 800
|term-1         = Stunned
|term-2         = Magic Skill
|term-3         = Movement Speed
}}
|-|Abyssal Form=
{{Ability
|variations-1   = Zenith
|name           = Garotte
|cooldown       = 12.0 / 11.4 / 10.8 / 10.2 / 9.6 / 9.0
|mana-cost      = 70 / 80 / 90 / 100 / 110 / 120
|skill-effect-1 = Blink
|skill-effect-2 = BUFF
|description    = Selena [[charges]] in a designated direction, dealing {{Scale|base=150-350|total-mp=50|md}} to enemies along the way. If this damage triggers {{b|"Abyssal Mark"|e}} effect, its [[CD]] resets immediately.
|ls-1           = Base Damage: 150 / 190 / 230 / 270 / 310 / 350
|term-1         = Magic Skill
|term-2         = Blink
|term-3         = CD
|notes=
*Despite being a magic skill, {{ai|Garotte}} does not trigger {{b|Magic Lifesteal}} but only triggers {{b|Spell Vamp}}.
}}</tabber>
=== Ultimate ===
<tabber>
|-|Elven Form=
{{Ability
|variations-1   = Zenith
|name           = Primal Darkness
|cooldown       = 4.5 / 4.0 / 3.5 / 3.0
|skill-effect-1 = Morph
|skill-effect-2 = Speed Up
|description    = Selena enters Abyssal Form and increases her [[Movement Speed]] by 30% for 0.8 seconds. Immediately resets the [[CD]] of {{ai|Soul Eater|"Soul Eater"}} and {{ai|Garotte|"Garotte"}}. {{b|Abyssal Form|e}}: Selena gains new skills. Her {{ai|Basic Attack}} will deal {{Scale|total-mp=25}} extra {{b|Magic Damage}}.
|ls-1           = Movement Speed Bonus: 30% / 35% / 40% / 45%
|term-1         = CD
|term-2         = Movement Speed
|term-3         = Magic Basic Attack
|notes=
*Her basic attack's extra damage only applies to one target; multiple targets hit will receive her basic [[physical damage]].
}}
|-|Abyssal Form= 
{{Ability
|variations-1   = Zenith
|name           = Blessing of the Moon Goddess
|cooldown       = 4.5 / 4.0 / 3.5 / 3.0
|skill-effect-1 = Morph
|skill-effect-2 = Speed Up
|description    = Selena enters Elven Form and increases her [[Movement Speed]] for 30%. Lasts 0.8 seconds. Immediately reset the [[CD]] of {{ai|Abyssal Trap|"Abyssal Trap"}} and {{ai|Abyssal Arrow|"Abyssal Arrow"}}.
|ls-1           = Movement Speed Bonus: 30% / 35% / 40% / 45%
|term-1         = Magic Skill
|term-2         = CD
|term-3         = Movement Speed
}}
</tabber>

== Gallery ==
=== Splash art ===
<gallery spacing="small" captionalign="center" hideaddbutton="true">
Selena (Abyssal Witch).jpg|Abyssal Witch
Selena (Wasp Queen).jpg|Wasp Queen
Selena (Virus).jpg|Virus
Selena (Gemini - Shadow).jpg|Gemini - Shadow
Selena (Double Identity).jpg|Double Identity
Selena (Thunder Flash).jpg|Thunder Flash
Selena (Gothic Curse).jpg|Gothic Curse
Selena (S.T.U.N. Selena).jpg|S.T.U.N. Selena
Selena (Lady Vengeance).jpg|Lady Vengeance
Selena (Curse of Cinder).jpg|Curse of Cinder
</gallery>

=== Avatar icons ===
<gallery spacing="small" captionalign="center" bordercolor="transparent" widths="100" hideaddbutton="true">
Hero631-icon.png|Abyssal Witch
Hero632-icon.png|Wasp Queen
Hero633-icon.png|Virus
Hero634-icon.png|Gemini - Shadow
Hero635-icon.png|Double Identity
Hero636-icon.png|Thunder Flash
Hero637-icon.png|Gothic Curse
Hero638-icon.png|S.T.U.N. Selena
Hero639-icon.png|Lady Vengeance
Hero106311-icon.png|Curse of Cinder
</gallery>
=== Battle emotes ===
<gallery spacing="small" captionalign="center" bordercolor="transparent" widths="100" hideaddbutton="true">
Abyss Twin.png|'''Abyss Twin'''<br>Battle Emote - 2019/10 Starlight Special
Our Song.png|'''Our Song'''<br>Obtained for free by collecting the 2021 515-series skins.
Sassy Selena.png|'''Sassy Selena'''<br>Can be purchased in the emote shop
Catch me!.png|'''Catch me!'''<br>Obtained via the Crimson Charm event.
Battle Emote - Troubled.png|'''Troubled'''<br>Obtained via the 2025 Curse of Cinder event.
</gallery>

== Videos ==
<gallery captionalign="center" spacing="small">
Mobile Legends Bang Bang! New Hero Abyssal Witch Selena
</gallery>
=== Skin spotlight ===
<gallery captionalign="center" spacing="small">
Selena New Skin - Double Identity - Mobile Legends- Bang Bang
Selena New Skin - Thunder Flash - Mobile Legends- Bang Bang
New Skin -Selena "Lady Vengeance" - Mobile Legends- Bang Bang
</gallery>

== Trivia ==
* Except from her basic skin "Wasp Queen", all of Selena's skins were first released as time-limited.
* The name '''Selena''' comes from the Greek name "Selene" (Σελήνη) or [[wikipedia:Selina|Selina]], which means "moon" or "goddess of the moon." In Greek mythology, [https://en.wikipedia.org/wiki/Selene Selene] was the Titan goddess of the moon, often depicted riding a chariot across the night sky.
* Selena was one of the first heroes in MLBB to have two separate skill sets in one data. Her switching mechanics make her both an [[Assassin]] and a [[Mage]], depending on the form.
* Selena was supposed to be released on the date {{hi|Chang'e}} was released, but the developers decided{{cite}} to postpone her release to a month later (following {{hi|Kaja}}) because some of them saw her as over-powered and others saw her under-powered.
* Her Abyssal Arrow (2nd Skill in Elven Form) can stun enemies for up to 3 seconds, making it one of the longest stuns in the game!
* Selena's Abyssal skin "Lady Vengeance" pays a wicked tribute to Starcraft’s Kerrigan.
* Selena's [[Zenith Skins|Zenith]] skin "Curse of Cinder" refers to Ashes. In Selena's Zenith skin back story her stepsister's calls her little Ashes. The name 'Cinder' is also a shortened form of '[https://en.wikipedia.org/wiki/Cinderella Cinderella],' from the classic fairy tale. Additionally, her outfit, theme, and skin story are inspired by it while {{hi|Vexana}}'s Zenith skin is inspired to [https://en.wikipedia.org/wiki/Snow_White Snow White].

== References ==
<references />
{{Heroes}}
{{Hero categories}}
[[Category:2019 heroes]]
[[Category:Heroes]]
[[Category:Jungler]]
[[Category:Abyss]]
[[Category:Azrya Woodlands]]
[[Category:Land of Dawn]]
[[Category:Dark Elf]]
[[Category:Elf]]
[[Category:Heroes with sidekicks]]
