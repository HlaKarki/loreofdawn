{{Roamer Note}}
{{clr}}
<p style="font-size:9pt;text-align:center;padding: 20px 0 15px 0;">There are currently '''36''' Heroes that are recommended to be a '''Roamer'''.</p>
<center>
{{Hero-box-xs|name=Akai}}
{{Hero-box-xs|name=Angela}}
{{Hero-box-xs|name=Atlas}}
{{Hero-box-xs|name=Badang}}
{{Hero-box-xs|name=Belerick}}
{{Hero-box-xs|name=Carmilla}}
{{Hero-box-xs|name=Chip}}
{{Hero-box-xs|name=Chou}}
{{Hero-box-xs|name=Diggie}}
{{Hero-box-xs|name=Edith}}
{{Hero-box-xs|name=Estes}}
{{Hero-box-xs|name=Franco}}
{{Hero-box-xs|name=Floryn}}
{{Hero-box-xs|name=Gatotkaca}}
{{Hero-box-xs|name=Gloo}}
{{Hero-box-xs|name=Grock}}
{{Hero-box-xs|name=Hilda}}
{{Hero-box-xs|name=Hylos}}
{{Hero-box-xs|name=Jawhead}}
{{Hero-box-xs|name=Johnson}}
{{Hero-box-xs|name=Kaja}}
{{Hero-box-xs|name=Kalea}}
{{Hero-box-xs|name=Khaleed}}
{{Hero-box-xs|name=Khufra}}
{{Hero-box-xs|name=Lolita}}
{{Hero-box-xs|name=Mathilda}}
{{Hero-box-xs|name=Minsitthar}}
{{Hero-box-xs|name=Minotaur}}
{{Hero-box-xs|name=Natalia}}
{{Hero-box-xs|name=Rafaela}}
{{Hero-box-xs|name=Saber}}
{{Hero-box-xs|name=Selena}}
{{Hero-box-xs|name=Tigreal}}
</center>

== Meta Roamers ==
It is recommended that you practice each roam hero at least once, so that you know enemy skill types, cooldowns, and learn matchups and recommended skill builds for different heroes. A Roamer will determine the initiations and pick offs that can change the tables in a match.

These heroes are currently meta and will dominate (most of the time as shown in tournaments) if you mastered them (In no particular order): {{Hero-box-xs|name=Chou}}{{Hero-box-xs|name=Mathilda}}{{Hero-box-xs|name=Gatotkaca}}{{Hero-box-xs|name=Kalea}}{{Hero-box-xs|name=Badang}}{{Hero-box-xs|name=Hylos}}

==List of unofficial roamers==
Note: Keep in mind these heroes were not officially listed as Roamers but were used as Roamers in higher ranks and pro tournaments.
*{{Hi|Baxia}}
*{{Hi|Arlott}}
*{{Hi|Helcurt}}
*{{Hi|Kadita}}

==Categories==
Roamers are "unofficially" divided into 4 different categories.
*Setter Roamers: Are roamers who have great combination of AOE and Crowd Control to initiate set up for the team. Tanks are part of this category.
**{{hi|Atlas}} 
**{{hi|Belerick}}
**{{hi|Carmilla}}
**{{hi|Gatotkaca}}
**{{hi|Johnson}}
**{{hi|Khaleed}}
**{{hi|Khufra}}
**{{hi|Minotaur}}
**{{hi|Tigreal}}
*Initiator Roamers: Are roamers who professionally engage to start up team fights by targeting a single or double enemies, however they lack a combination of strong crowd control and AOE. Fighters and some Tanks are part of this category.
**{{hi|Akai}}
**{{hi|Badang}}
**{{hi|Chip}}
**{{hi|Chou}}
**{{hi|Edith}}
**{{hi|Franco}}
**{{hi|Gloo}}
**{{hi|Grock}}
**{{hi|Hilda}}
**{{hi|Hylos}}
**{{hi|Kaja}}
**{{hi|Kalea}}
**{{hi|Mathilda}}
**{{hi|Minsitthar}}
*Utility/Healing Roamers: Are roamers who use their abilities to protect their allies from debuffs. Supports are part of this category.
**{{hi|Angela}}
**{{hi|Estes}}
**{{hi|Floryn}}
**{{hi|Lolita}}
**{{hi|Rafaela}}
*Damage Roamers: Are roamers who specialize high damage to distract the enemy and target their backlines but are squishy and must require a frontline Jungler or EXP Laner to protect them. Assassins are part of this category.
**{{hi|Natalia}}
**{{hi|Saber}}
**{{hi|Selena}}

[[Category:Heroes]]
[[Category:Roamer]]
[[Category:Laning Recommendations]]