{{Tabs data}}
{{Infobox data}}
{{Hero stats}}
==Story==
{{Infobox data story
| full_name      = 
| alias          = Fortress Titan
| born           = 
| origin         = [[Lantis Mountains]], [[The Barren Lands]]
| species        = Castle
| gender         = Male
| title          = 
| occupation     = Guardian
| affiliation    =
*Ancient Ones
*[[The Abyss]] (hostile)
| fighting_style = 
| weapons        = Weathered Citadel
| abilities      = *[[#Skill 1|Power of nature]]
*Summons [[#skill 2|guardian's barrier]]
| religion       =
| battles_fought =
| height         = 
| weight         = 
| traits         = 
| likes          = Sleeping
| dislikes       = 
| relationships  = 
|en_va=
|id_va=
|ja_va= [https://www.animenewsnetwork.com/encyclopedia/people.php?id=150106 Nakamura Taishi (中村 大志)]<ref name="In-game credit">In-game credit</ref>
|tl_va=
|ar_va=
|pt_va=
|ru_va=
|tk_va=
|es_va=
|zh_va=
}}
{{Quote|Stone man awakened from sleep}}
The stone man who signed a contract with the Ancient Ones has a kind-hearted nature and infinite power. He sleeps most of his life. After Grock woke up, he found that the Ancient Ones he was guarding had disappeared, so he carried the Castle of the Ancient Ones on his shoulders and walked out of the mountains to find the whereabouts of the Ancient Ones.
=== Lore ===
Deep within the mountains of the [[Land of Dawn]] sits an ancient fortress. Though the owner is unknown, it is rumored that it could be a descendant of the legendary race that once walked the Lands of Dawn. It said that this race is older than the earth itself, and having a special affinity for nature, they were able to build deep connections and relations with various beings of nature, especially the Stone Titans.

These amiable giants possess rocky skin that is almost impervious to all physical force, but due to their kind nature, they mainly use their physical prowess to sleep safely. These stony giants love sleeping so much that they will hibernate for up to half a year sometimes, however once they are awakened, it is hard for them fall asleep again. One day when Grock awoke again, he found himself to be alone with the guardians of the fortress long gone. He proceeded to saddle the fortress onto his back to find the owners, setting out all over the Land of Dawn to find any news of their whereabouts, so he may one day return their home to them.

==Abilities==
===Passive===
{{Ability
|name           = Bastion of Stone
|skill-effect-1 = BUFF
|description    = Grock gains a [[Shield]] equal to 10% of his Max HP and empowers the next Basic Attack to deal {{scale|pd}} equal to {{scale|base=4|total-pdef=2}}% of the target's Max HP (capped at {{scale|base=300|level=20}} against non-data units). This effect has a 18-12 seconds cooldown (decreases with level), which reduces by 200% when near terrain. <br> Grock gains [[Control Immunity]] while casting skills and {{scale|base=30|level=4}}% extra [[Physical Defense]] while the Shield is active.
|term-1         = Physical Defense
|term-2         = Control Immunity
|term-3         = 
}}

===Skill 1===
{{Ability
|name           = Mighty Swing
|cooldown       = 8.0 / 7.6 / 7.2 / 6.8 / 6.4 / 6.0
|skill-effect-1 = CC
|skill-effect-2 = AoE
|skill-type-1   = Physical Skill
|description    = Grock strikes forward with massive force, dealing {{scale|base=240-440|extra-pa=100|pd}}. Enemies with obstacle, launched [[airborne]], and take extra {{scale|pd}} equal to {{scale|base=4|total-pdef=2}}% of their Max HP (capped at {{scale|base=300|level=20}} against non-data units). If there are no obstacles behind, enemies are only [[knocked back]] slightly.
|ls-1           = Base Damage: 240 / 280 / 320 / 360 / 400 / 440
|term-1         = Airborne
|term-2         = 
}}
===Skill 2===
{{Ability
|name           = Earthen Rampart
|cooldown       = 14.0 / 13.2 / 12.4 / 11.6 / 10.8 / 10.0
|skill-effect-1 = CC
|skill-effect-2 = AoE
|skill-type-1   = Physical Skill
|description    = After a brief delay, Grock summons a stone wall in the target area, dealing {{scale|base=100-200|extra-pa=60|pd}} and [[knocked back|knocking]] enemies towards him. <br> {{link|Use Again}}: Cancel the wall early.
|ls-1           = Base Damage: 100 / 120 / 140 / 160 / 180 / 200
|term-1         = 
|notes           = 
*Placing a wall near a nearby unit (including himself) will displace them while disrupting their skills, whether it is an ally or an enemy and even if they are [[Control and slowing immunity|immune]] to [[crowd control]]. Because the displacing effect is not counted as a control effect, but rather as a result of the way obstacles work, any units who can manage to place themselves on the obstacle's way will automatically be moved on the valid ground (except when flying over or ignores an obstacle until the duration ends) to avoid becoming stuck. The same effect applies to {{hi|Badang}}'s {{ai|Fist Break}}.
}}                       
===Ultimate===
{{Ability
|name           = Tectonic Charge
|quote          = The brave fear nothing.
|cooldown       = 55.0 / 50.0 / 45.0
|skill-effect-1 = Mobility
|skill-effect-2 = AoE
|skill-type-1   = Physical Skill
|skill-type-2   = Charge
|description    = Grock charges forward, dealing {{scale|base=100|total-pa=50|pd}} to enemies in his path. Upon hitting terrain, he immediately stops and triggers a massive explosion that knocks nearby enemies [[airborne]] for 1 second and deals {{scale|650-1050|total-pa=300|pd}}. If this skill doesn't hit terrain, 50% of its cooldown is refunded.
|ls-1           = Base Damage: 100 / 100 / 100
|ls-2           = Added Damage: 650 / 850 / 1050
|term-1         = Airborne
|term-2         = Charge
|term-3         = Cooldown
}}
==Gallery==
===Artwork===
<gallery captionalign="center" spacing="small" hideaddbutton="true">
Grock (Fortress Titan).jpg|Fortress Titan
Grock (Grave Guardian).jpg|Grave Guardian
Grock (Castle Guard).jpg|Castle Guard
Grock (V.E.N.O.M. Monitor Lizard).jpg|V.E.N.O.M. Monitor Lizard
File:Iceland Golem wall.jpg|Iceland Golem
Grock (Codename Rhino).jpg|Codename: Rhino
Grock (Ancient Golem).jpg|Ancient Golem
File:Wasteland Psycho .jpg|Wasteland Psycho
</gallery>
===Avatar icons===
<gallery captionalign="center" spacing="small" widths="100" bordercolor="transparent" hideaddbutton="true">
Hero441-icon.png|Fortress Titan
Hero442-icon.png|Grave Guardian
Hero443-icon.png|Castle Guard
Hero444-icon.png|V.E.N.O.M. Monitor Lizard
Hero445-icon.png|Iceland Golem
Hero446-icon.png|Codename: Rhino
Hero447-icon.png|Ancient Golem
Hero448-icon.png|Wasteland Psycho
</gallery>
===Battle emotes===
<gallery captionalign="center" spacing="small" widths="100" bordercolor="transparent" hideaddbutton="true">
Weapon Care.png|'''Weapon Care'''<br>Battle Emote - 2019/11 Starlight Special
</gallery>

===Hero spotlight===
<gallery captionalign="center" spacing="small">
Mobile Legends Bang bang! New Hero Fortress Titan Grock Gameplay
</gallery>

== Trivia ==
* Grock’s design is heavily influenced by golems, which are stone or earth-based protectors in mythology and fantasy. 
* Grock is the first data to summon a physical wall in the game. 

== Navigation ==
{{Heroes}}
