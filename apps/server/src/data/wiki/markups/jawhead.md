{{Tabs data}}
{{Infobox data}}
{{Hero stats}}
== Story ==
{{Infobox data story
| name            = 
| chinese_name    = 大铁罐 (dàtiěguàn, "Great Metal Pot"<ref>大 (dà) meaning "big, large, huge, great, heavy, main, major, well-known, successful"<br>铁罐 (tiěguàn) meaning "metal pot"</ref>)
| image           = Guardian p8.jpg
| full_name       = SMART-01
| alias           = *Steel Sweetheart
*Prototype One
*Jawhead (by Ailee)
| born            = 
| origin          = [[Laboratory 1718]]
| age             = 
| birthday        = {{birthday|6|11}}<ref name="CNProfile">Official Profile from MLBB CN</ref>
| species         = Robot
| gender          = Ungendered
| title           = 
| occupation      = Guardian of Henry family
| fields_of_study = 
| affiliation     = 
| fighting_style  = 
| weapons         = *[[#Skill 1|Smart missiles]]
| abilities       = 
| religion        =
| nationality     = 
| battles_fought  = 
| height          = 
| weight          = 
| traits          = 
| likes           = 
| dislikes        = 
| relationships   = *Prof. and Mrs. Henry (creator)
| en_va           = 
| ja_va           = Kontani Yuta (紺谷 勇太)<ref>In-game credit</ref>
| id_va           = 
| tl_va           = 
| ar_va           = 
| pt_va           = 
| ru_va           = 
| tk_va           = 
| es_va           = 
| zh_va           = 
}}
{{Infobox character
| name            = Ailee
| image           = 
| full_name       = Ailee Henry
| alias           = Honey/Honeybear (by Jawhead)
| born            = 
| origin          = 
| age             = 
| birthday        = {{birthday|6|11}}<ref name="CNProfile"/>
| species         = Human
| gender          = 
| title           = 
| occupation      = 
| fields_of_study = 
| affiliation     = 
| fighting_style  = 
| weapons         = 
| abilities       = 
| religion        =
| nationality     = 
| battles_fought  = 
| height          = 
| weight          = 
| traits          = 
| likes           = 
| dislikes        = 
| relationships   = *Prof. and Mrs. Henry (parents)
| en_va           = 
| ja_va           = 
| id_va           = 
| tl_va           = 
| ar_va           = 
| pt_va           = 
| ru_va           = 
| tk_va           = 
| es_va           = 
| zh_va           = 
}}
{{quote|Intelligent robot used to protect little [Ailee]}}
This intelligent robot developed by Henry and his wife is armed with weapons and strong defensive capabilities, and can also act as a housekeeper. It is responsible for protecting the couple's daughter little Ailee after their disappearance.
=== Lore ===
Clandestine organizations have existed throughout the many ages of the Land of Dawn's history. No one knows the true purpose of these organizations, but one thing is for sure: whenever a crisis is at hand, mysterious figures emerge from the shadows to restore order to the world.

Professor Henry and his wife worked for the Dawn Council, known for its research in cutting-edge technology, providing an endless and varied stream of high-tech products to government agencies and mysterious, anonymous clients. But when they finally finished developing the "SMART-01", all this changed, as their new creation was more intelligent and powerful than any android the world had seen before. It was designed so that once a genetically matched host controlled him, he could act according to the controller's consciousness. The leaders of the Dawn Council saw the potential of this technology for warfare and ordered the Henrys to transfer the technology to selected government agencies for a mass production program. The Henrys refused.

After ordering their Prototype One to destroy all of the experiment's data, the Henrys mysteriously disappeared. To this day, little Ailee has no knowledge of these events. All she knows and cares about is that this special robot has always been there to protect and accompany her, and that he calls her "Honey" like her father once did, while she affectionately calls him "Jawhead" in return. The only clue left behind by her parents was a pocket watch she carries with her wherever she goes. Eventually, Ailee decides to journey with her robot friend to track down her parents' whereabouts...
=== Guardian ===
{{main|Guardian}}
{{Clr}}

== Abilities ==
=== Passive ===
{{Ability
|name            = Mecha Suppression
|skill-effect-1  = Buff
|skill-effect-2  = 
|cooldown        = 
|mana-cost       = 
|quote           = 
|description     = Jawhead's attacks apply a stack of {{b|Mecha Suppression|e}} on hit (up to 10 stacks). Each stack lasts 3 seconds and increases the damage of Jawhead's {{ai|Basic Attacks}} to the enemy by 8%.
|term-1          = Physical Basic Attack
|term-2          = 
|term-3          = 
|calc            = 
|stack-scaling   = Basic Attack Damage Increase Per Stack: 8% / 16% / 24% / 32% / 40% / 48% / 56% / 64% / 72% / 80%
|notes           = *Only applies to non-[[minion]] enemies.
*Jawhead's attacks also refresh the duration of the stacks. 
*"Attacks" that apply Mecha Suppression stacks only refer to Jawhead's basic attack and skill damage dealt.
|bugs            = 
}}
=== Skill 1 ===
{{Ability
|name            = Smart Missiles
|skill-effect-1  = Burst
|skill-effect-2  = 
|cooldown        = 5.0
|mana-cost       = 60 / 65 / 70 / 75 / 80 / 85
|quote           = 
|description     = Jawhead launches up to 12 missiles at random nearby enemies over 5 seconds, each missile dealing {{scale|base=135-260|total-pa=30|pd}} to the enemy hit.
|term-1          = Physical Damage Skill
|term-2          = 
|term-3          = 
|calc            = 
|level-scaling   = Base Damage: 135 / 160 / 185 / 210 / 235 / 260
|notes           = 
|bugs            = 
}}
=== Skill 2 ===
{{Ability
|name            = Ejector
|image-2         = Ejector 2
|skill-effect-1  = Shield
|skill-effect-2  = CC
|cooldown        = 15.0 / 14.4 / 13.8 / 13.2 / 12.6 / 12.0
|mana-cost       = 80 / 90 / 100 / 110 / 120 / 130
|quote           = 
|description     = Jawhead gains 30% [[Movement Speed]] and a {{scale|base=250-600|total-pa=180|shield}} for S seconds.<br>{{B|Use Again:|e}} Jawhead [[airborne|flings]] the nearest unit (prioritizes heroes) to the target location, dealing {{scale|base=300-500|total-pa=80|pd}} to enemies in the area and [[stunning]] them for 0.5 seconds. Can used on allied heroes and they will not take damage.
|term-1          = Stun
|term-2          = Movement Speed
|term-3          = Shield
|calc            = 
|level-scaling   = Base Shield: 250 / 320 / 390 / 460 / 530 / 600
Base Damage: 300 / 340 / 380 / 420 / 460 / 500
|notes           = 
|bugs            = 
}}
=== Ultimate ===
{{Ability
|name            = Unstoppable Force
|skill-effect-1  = Mobility
|skill-effect-2  = CC
|cooldown        = 35.0 / 30.0 / 25.0
|mana-cost       = 120 / 150 / 180
|quote           = 
|description     = Jawhead [[blink|charges]] into the target enemy data, dealing {{scale|base=350-600|total-pa=150|pd}} and briefly [[stunning]] them.<sup>[''duration unknown'']</sup> Enemies around the target are [[knocked back]] and dealt the same amount of the damage. Jawhead is [[Control and slowing immunity|immune to control effects]] during the charge.
|term-1          = Stun
|term-2          = Blink
|term-3          = Displacement
|calc            = 
|level-scaling   = 
Base Damage: 350 / 475 / 600
|notes           =*This ability has three phases:
**{{hi|Jawhead}} begins to channel the charge. If interrupted at this step, the ability does not go on cooldown.
**{{hi|Jawhead}} travels to the target.
**Upon hitting the target, {{hi|Jawhead}} loses CC immune and nearby enemy is knocked back. 
|bugs            = 
}}
== Gallery ==
===Splash arts===
<gallery spacing="small" captionalign="center" hideaddbutton="true">
Jawhead (Steel Sweetheart).jpg|Steel Sweetheart
Jawhead (Girl Scout).jpg|Girl Scout
Jawhead (Space Explorer).jpg|Space Explorer
Jawhead (Foo Quarterback).jpg|Foo Quarterback
Jawhead (The Nutcracker).jpg|The Nutcracker
Jawhead (Samurai Mech).jpg|Samurai Mech
Jawhead (Candy Bear).jpg|Candy Bear
Jawhead (Cyber Ranger).jpg|Cyber Ranger
Jawhead (Victory Loader).jpg|Victory Loader
Jawhead (Proto Guardian).jpg|Proto Guardian
</gallery>
;Painted skins
<gallery spacing="small" captionalign="center" hideaddbutton="true">
Jawhead (Proto Sentinel).jpg|Proto Sentinel
</gallery>

===Avatar icons===
<gallery bordercolor="transparent" captionalign="center" spacing="small" widths="100" hideaddbutton="true">
Hero541-icon.png|Steel Sweetheart
Hero542-icon.png|Girl Scout
Hero543-icon.png|Space Explorer
Hero544-icon.png|Foo Quarterback
Hero545-icon.png|The Nutcracker
Hero546-icon.png|Samurai Mech
Hero547-icon.png|Candy Bear
Hero548-icon.png|Cyber Ranger
Hero549-icon.png|Victory Loader
Hero105410-icon.png|Proto Guardian
</gallery>
;Painted skins
<gallery captionalign="center" spacing="small" widths="100" bordercolor="transparent" hideaddbutton="true">
Hero105410-color01-icon.png|Proto Sentinel
</gallery>
=== Battle emotes ===
{{Emote|Jawhead}}

== Trivia ==
*Jawhead is the only data who carries a companion (Ailee) that does not help or assist in fight.
*Revisions of the name of Ailee was made from Alice and Ellie.
*Almost all of Jawhead's skins has two eyes/lens which is different from default's one.
*'''Victory Loader''' is the first skin that use Ailee's face in the avatar icon instead of using Jawhead's face.

==References==
{{Reflist}}

==Navigation==
{{Heroes}}
